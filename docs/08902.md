...
> 原文:[https://dev . to/app signal/under-the-hood-slurping-and-streaming-files-in-ruby-h24](https://dev.to/appsignal/under-the-hood-slurping-and-streaming-files-in-ruby-h24)

在这一期的 Ruby Magic 中，我们将了解 Ruby 中的流文件，`IO`类如何处理读取文件而不完全加载到内存中，以及它如何通过缓冲读取字节来读取每行文件。让我们开始吧！

## [](#slurping-and-streaming-files)【啧啧有声】和流式文件
...
## [](#io-and-rubys-raw-io-endraw-class)I/O 和 Ruby 的`IO`类
...
## [](#reading-io-streams-via-the-kernel)通过内核读取 I/O 流
...
> ## [](#file-descriptors-and-the-file-table)文件描述符和文件表
> 
...
### [](#reading-bytes-and-moving-cursors)读取字节和移动光标
...
## [](#reading-files-line-by-line)逐行读取文件
