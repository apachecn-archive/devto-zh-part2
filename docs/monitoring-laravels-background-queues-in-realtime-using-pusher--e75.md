# 使用 Pusher 实时监控 Laravel 的后台队列

> 原文：<https://dev.to/neo/monitoring-laravels-background-queues-in-realtime-using-pusher--e75>
...
**设置数据库连接和迁移**
安装完成后，我们可以继续设置数据库。打开`.env`文件，替换下面的配置:
...
在终端`cd`中找到你的项目的根目录。运行下面的命令来创建 SQLite 数据库文件:
...
**设置邮件陷阱进行邮件测试**
打开您的`.env`文件，输入您从邮件陷阱仪表板上获得的密钥。下面列出了相关的键:
...
这将生成一个身份验证框架。这就是关于身份验证您需要做的全部工作。

**配置推送器**
将`.env`文件中的`PUSHER_*`键替换为您从推送器仪表板上获得的正确键:
...
安装完成后，打开`config/broadcasting.php`文件并滚动到`pusher`部分。将`options`键替换为以下内容:
...
> 💡在生产环境中，最好使用像 Redis 或 Memcached 这样的内存存储作为队列驱动程序。内存存储比使用关系数据库更快，因此性能更好。
...
这将在`app`目录中创建一个新模型。打开文件，将内容替换为以下内容:
...
接下来，打开`HomeController`和，用下面的代码替换内容:
...
在上面的控制器中，我们有 4 个方法，大部分都是不言自明的。在这个类中，我们使用 [Faker 库](https://github.com/fzaninotto/Faker)，它帮助我们生成随机的假值。在`simulate`方法中，我们使用 faker 库来生成一个假的电子邮件地址和描述。我们实例化一个`SimulateMail` [可邮寄的](https://laravel.com/docs/5.5/mail#generating-mailables)。
...
在上面的类中，我们有一个构造函数，它在`queued_emails`表中创建了一个新条目。在`build`方法中，我们构建将要发送的邮件消息。

在`send`方法中，我们将`queued_emails`条目的`run`列标记为`true`。我们还发起了一个名为`EmailSent`的活动。在`queue`方法中，我们还触发了一个叫做`EmailQueued`的事件。
...
这个类和`EmailSent`事件类几乎一样。微小的区别是`broadcastAs`方法。它返回一个不同的别名来广播事件。

最后，打开 routes 文件`routes/web.php`并将代码替换为:
...
既然我们已经设置了大部分后端，我们将创建应用程序的前端。打开`resources/views/home.blade.php`文件，用以下代码替换代码:
...
这将安装 [Laravel Echo](https://laravel.com/docs/5.5/broadcasting#installing-laravel-echo) 和[推进器 JS SDK](https://github.com/pusher/pusher-js) 。安装完成后，运行下面的命令来安装其他 NPM 依赖项:
...
**构建我们的 Vue 组件**
让我们构建我们之前提到的`jobs` Vue 组件。打开`resources/assets/js/app.js`文件，替换下面的代码:
...
在上面的 Vue 组件中，我们定义了一个`template`。在那里，我们循环遍历`jobs`数组并列出每个作业的描述和时间戳。
...
同样的方法，我们使用 [Laravel Echo](https://laravel.com/docs/5.5/broadcasting#installing-laravel-echo) 监听 Pusher 通道并等待事件被触发。每当事件`.add`和`.sent`被触发时，我们就调用`refreshAllJobs`方法。

> 💡**事件名称前面有一个点，因为，****** * *在 Laravel 中，每当使用** `**broadcastAs**` **方法定义别名** **时，都需要加上点* * * *。如果没有点，你的事件将不会被听众捕捉到。如果不提供别名，Laravel 将使用 namespace + class 作为广播事件的名称。**

打开`resources/assets/js/bootstrap.js`文件。在文件的底部，添加以下代码:
...
> ⚠️ **确保用你的 Pusher 应用键和集群替换** `**PUSHER_APP_KEY**` **和** `**PUSHER_APP_CLUSTER**` **。**
...
这将创建一个 PHP 服务器，这样我们就可以预览我们的应用程序。将在终端上提供 URL，但默认为 [http://127.0.0.1:8000](http://127.0.0.1:8000) 。

当您看到 Laravel 主页时，使用页面右上角的“注册”链接创建一个新帐户。现在，单击“模拟”按钮，您应该会看到一个新的排队电子邮件条目。

现在我们将使用`queue:listen` artisan 命令手动执行队列中的流程。打开一个新的终端窗口，运行下面的命令:
...
这应该开始执行它看到的任何队列。只要终端打开并且`queue:listen`命令正在运行，当您点击“模拟”按钮时，队列将立即运行。如果您取消了`queue:listen`命令，队列条目将保留在那里，不会被触发。

> 💡**在生产环境中，您不能保持** `**queue:listen**` **运行，您可能需要一个工人在后台进程上运行；类似于** [**主管**](http://supervisord.org/index.html) **。你可以在这里 **阅读更多关于你如何做的** [**。**](https://laravel.com/docs/5.5/queues#running-the-queue-worker)**
...
这篇文章最初出现在 [Pusher 博客](https://blog.pusher.com/monitoring-laravel-background-queues-realtime-pusher/)上。