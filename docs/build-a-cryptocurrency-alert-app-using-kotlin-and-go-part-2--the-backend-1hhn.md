...
> 原文：<https://dev.to/neo/build-a-cryptocurrency-alert-app-using-kotlin-and-go-part-2--the-backend-1hhn>

> 你需要在你的机器上安装 Android Studio 3+和 Go 1.10.2+。你应该熟悉 Android 开发和 Kotlin 语言。

在本文的第一部分中，我们通过创建我们的 Android 应用程序开始构建我们的服务。然而，该应用程序需要后端才能正常工作。所以在这一部分，我们将创建应用程序的后端。
...
在上面的代码中，我们首先导入了一些包，Go 脚本将需要这些包才能工作。然后，我们使用导入的`database`子包实例化数据库。接下来，我们在`db`实例上运行迁移。这将创建应用程序需要运行的数据库表(如果它还不存在)。
...
> Logger 中间件记录关于每个 HTTP 请求的信息。
> *恢复中间件从链中任何地方的死机中恢复，打印堆栈跟踪并将控制处理给集中*[httperrhandler](https://echo.labstack.com/guide/customization#http-error-handler)*。*
...
3.  `POST /eth-pref` -存储在收到通知并返回 JSON 响应之前 ETH 必须超过的最低和最高价格。
4.  `GET /simulate` -模拟受支持货币的价格变化。

完成路由后，我们在端口 9000 上启动服务器。

> *如果正在使用 9000，您可以选择一个不同的端口，只要记住也在您的*`MainActivity.kt`*文件中更改它。*
...
如前所述，我们将创建一些内部包来使应用程序更加模块化，所以让我们从`database`包开始。
...
在上面的文件中，我们首先导入两个包，一个是内置的`database/sql`，另一个是 [mattn/go-sqlite3](http://github.com/mattn/go-sqlite3) 包，它是使用`database/sql`的 go 的 sqlite3 驱动程序。打开终端并运行以下命令:
...
首先，我们将添加`GetPrices`函数。在同一文件的底部粘贴以下代码:
...
> *Go 不允许在结构名中使用下划线，所以我们将使用* `json:` `* " *`key_name`* " `格式来自动与指定键的属性进行相互转换。*
...
在上面的`GetSettings`函数中，我们创建了一个空的`Device`结构。我们运行查询来从`devices`表中获取与`uuid`匹配的设备。然后，我们使用数据库包的`Scan`方法将行值保存到`Device`实例中。

如果没有找到设备，使用我们之前创建的`CreateSettings`函数创建一个新的设备，否则返回找到的设备。
...
在上面的函数中，我们创建了一个新的`coinPrice`实例，然后我们创建了一个数组，其中包含我们想要获取的两种货币，ETH 和 BTC。然后我们遍历货币，如果`simulate`是`true`，我们就返回硬币的模拟价格。如果是`false`，那么对于每种货币，我们执行以下操作:
...
第二个函数是`NotifyDevicesOfPriceChange`函数。在这里，我们循环遍历货币价格，对于每个价格，我们检查数据库中匹配最低和最高价格的设备。
...
在`notifications`目录中，创建一个`push.go`文件并粘贴以下代码:
...
我们需要做的最后一件事是将推杆组件拉入我们的`$GOPATH`。打开您的终端并运行以下命令:
...
现在最小化模拟器中的应用程序，打开通知中心。访问 URL[http://localhost:9000/simulate](http://localhost:9000/simulate)来模拟货币变化。您应该会看到通知进入设备，如下所示:
...
这篇文章最初出现在 [Pusher 博客](https://pushcomer./tutorials/cryptocurrency-kotlin-go-part-2)上。