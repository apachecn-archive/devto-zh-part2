...
## [](#how-to-use-metamask-for-a-oneclick-login-flow)如何为一键登录流程使用元掩码
...
## [](#the-metamask-browser-extension)元掩码浏览器扩展
...
## [](#how-the-login-flow-works)登录流程如何工作
...
### [](#step-1-modify-the-user-model-backend)第一步:修改用户模型(后端)
...
### [](#step-2-generate-nonces-backend)第二步:生成随机数(后端)

对于数据库中的每个用户，在`nonce`字段中生成一个随机字符串。比如`nonce`可以是一个大的随机整数。

### [](#step-3-user-fetches-their-nonce-frontend)步骤 3:用户获取他们的 Nonce(前端)
...
### [](#step-4-user-signs-the-nonce-frontend)第四步:用户签署随机数(前端)
...
### [](#step-5-signature-verification-backend)第五步:签名验证(后端)
...
## [](#lets-build-it-together)让我们一起建造吧
...
## [](#let-your-users-login-with-metamask)让您的用户使用元掩码登录
