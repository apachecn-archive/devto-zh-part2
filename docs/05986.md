...
> 原文:[https://dev.to/jamesmh/asyncawait-for-the-rest-of-us-c8a](https://dev.to/jamesmh/asyncawait-for-the-rest-of-us-c8a)
...
## [](#for-desktopapp-developers)面向桌面/应用开发者

但是桌面应用程序不处理 HTTP 请求...

桌面应用程序**处理用户输入(键盘、鼠标等)。)、动画等。而且只有**一个 UI 线程**来做这件事。**

### [](#user-input-is-user-input)用户输入就是用户输入
...
### [](#when-does-this-happen)这是什么时候的事？
