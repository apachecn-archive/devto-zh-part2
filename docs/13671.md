...
#### [](#-raw-s3-endraw-filesystem)`s3://`文件系统
...
#### [](#-raw-s3n-endraw-filesystem)`s3n://`文件系统
...
#### [](#-raw-s3a-endraw-filesystem)`s3a://`文件系统
...
### [](#testing-s3a-with-hcp-7x-no-multipart-support)使用 HCP 7.x 测试 S3A(不支持多部分)
...
### [](#testing-s3a-with-hcp-8x-with-multipart-support)使用 HCP 8.x 测试 S3A(支持多部分)
