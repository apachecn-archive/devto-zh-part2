# 关于软件开发的有趣事实:统计 2018

> 原文：<https://dev.to/apium_hub/interesting-facts-about-software-development-statistics-2018-1e5k>

今年 [Apiumhub](/) 与[codigans](https://codingsans.com/blog/software-development-trends-2018?msID=c04e1da2-6098-402f-b5f9-f5ec73da7f9c)和其他软件相关公司合作，如: [instabug](https://instabug.com/) 、 [clutch](https://clutch.co/) 、[shipped](https://www.shippable.com/)、 [codegiant](https://codegiant.io/home) 、 [stride](https://www.stridenyc.com/) 、 [codeship](https://codeship.com/) 、 [usersnap](https://usersnap.com/) 、 [gitkraken](https://www.gitkraken.com/) ，并参与了一项全球软件开发研究，以了解如何我们收到了来自世界各地不同国家的 300 多个答案，我们真心希望您会对这份报告感兴趣。要获得关于软件开发有趣事实的完整报告，点击[这里](https://codingsans.com/blog/software-development-trends-2018?msID=c04e1da2-6098-402f-b5f9-f5ec73da7f9c)。

## 先睹为快:关于软件开发的有趣事实

现在，让我们来看看报告中的一些数据，了解一下这份报告的内容。让我强调一下，在这篇文章中，你不会找到每个图表和问题的详细解释。只是先睹为快。

众所周知，软件开发是一个快速变化的领域。几乎每年我们都会看到新的语言、软件架构、容器、方法等等。今天高度使用的工具或语言可能在明年就过时了。今年，我们的目的是对软件开发的当前状态拍一张快照，并对它的发展方向做出结论。

这份报告有不同的章节，报告的第一部分是关于软件团队面临的挑战和他们试图实现的解决方案。在这一部分中，您还会发现最流行的编程语言和项目管理工具。

报告的第二部分是关于招聘和留住人才的挑战和解决方案。是科技公司面临的最大挑战之一。最后一章关注性能管理和代码质量。您将从开发人员和经理的角度看到交付问题的原因。

说实话，我们非常兴奋地向您展示 2018 年软件开发的有趣事实。我们希望你会喜欢它！

好，我们开始吧！

## 关于软件开发的有趣事实

**1。软件开发的挑战**

科技公司面临的最大挑战与产能有关:在积压订单满满、产能有限的情况下交付工作软件。紧随这一挑战而来的是招聘。我们可以说，吸引和留住人才是软件开发中的一个巨大问题。但是团队如何克服能力挑战呢？最流行的解决方案是优化团队的工作流程。之后是优先化和增加项目工作人员的数量。此外，应用敏捷方法是应对能力挑战的一种常见方式。团队如何解决招聘问题？最流行的方法是雇佣内部招聘人员或外部猎头来接触潜在的候选人。另一种方法是启动实习项目。要查看图表和其他解决方案，请点击[此处](https://codingsans.com/blog/software-development-trends-2018?msID=c04e1da2-6098-402f-b5f9-f5ec73da7f9c)。

**2。编程语言**

最常用的编程语言有哪些？以及 2019 年会使用哪些语言？答案都在这里: **[![popular Programming languages](img/6342dc95723f4aa65f20ee2862c39bac.png)](https://apiumhub.com/wp-content/uploads/2018/07/Screen-Shot-2018-07-06-at-14.10.10.png)** 

*来源:[软件发展状况报告](http://codingsans.com/state-of-software-development-2018)*

我们可以看到使用最广泛的编程语言是 JavaScript，60.73%。奇怪的是，36.63%的受访者表示，他们不打算在未来 12 个月内使用任何新的编程语言。

**3。工具**

这份报告清楚地表明，团队使用各种各样的工具，尤其是在测试和项目管理方面。如果你看一下图表，你会发现“其他”类别远远高于任何其他类别。

[![testing tools](img/bba2a5c40972ab7bbeed037186b1a3f7.png)T2】](https://apiumhub.com/wp-content/uploads/2018/07/Screen-Shot-2018-07-06-at-14.17.33.png)

*来源:[软件发展状况报告](http://codingsans.com/state-of-software-development-2018)*

要了解更多关于最流行的项目管理和沟通工具，请点击[此处](https://codingsans.com/blog/software-development-trends-2018?msID=c04e1da2-6098-402f-b5f9-f5ec73da7f9c)。

**4。雇佣和留住人才**

正如我们所见，招聘是软件团队的第二大挑战，也是经理的第一大挑战。本章是关于科技公司的招聘实践，包括招聘方法、标准以及吸引和激励人才。

[![hiring criteria software development](img/087608791d26887e2d3eab03c60f14f0.png)T2】](https://apiumhub.com/wp-content/uploads/2018/07/Screen-Shot-2018-07-06-at-14.20.21.png)

*来源:[软件发展状况报告](http://codingsans.com/state-of-software-development-2018)*

从我们可以看到，很明显，科技公司在招聘人才时关注 4 个重要领域:学习意愿、文化契合度、工作经验和技术技能评估测试。

奇怪的是，现在很少有公司看学位，它成了次要的东西。最重要的一点是愿意学习，这是有意义的，因为我们生活在一个快速变化的世界，人们应该能够适应新的环境，找到新的机会，把工作做得更好。

至于留住人才，让我们看看图表，找出公司用来留住人才的主要因素是什么:

[![tacktics to attract software developers](img/d907647df8e494b4838b4bafdb6f67de.png)T2】](https://apiumhub.com/wp-content/uploads/2018/07/Screen-Shot-2018-07-06-at-14.22.06.png)

*来源:* [*软件发展状况报告*](http://codingsans.com/state-of-software-development-2018)

事实证明，富有挑战性的工作和优秀的团队文化是保持开发人员积极性的最受欢迎的方式。

**5。软件外包**

由于处理能力问题是招聘人才面临的最大挑战之一，外包可能是一个快速解决办法。让我们来看看公司是如何看待外包的:

[![software outsourcing statistics](img/98d8c467c96dcac1e9e8ef49e3063efd.png)T2】](https://apiumhub.com/wp-content/uploads/2018/07/Screen-Shot-2018-07-06-at-14.23.53.png)

_ 资料来源:*[_ 软件发展状况报告*](【http://codingsans.com/state-of-software-development-2018】T2)

嗯，正如我们所看到的，大多数外包软件开发服务的人还是比较满意的。这可能是容量问题的一个很好的解决方案。

**6。绩效管理**

在报告的这一部分，我们将看看软件团队如何度量性能，以及是什么导致了交付问题。这一部分还揭示了开发团队如何确保代码质量。

[![metrics to measure software developers' performance](img/78516eb2a9593917892cc1dc427c41a8.png)T2】](https://apiumhub.com/wp-content/uploads/2018/07/Screen-Shot-2018-07-06-at-14.25.28.png)

_ 资料来源:*[_ 软件发展状况报告*](【http://codingsans.com/state-of-software-development-2018】T2)

正如我们所看到的工作软件，完成的任务和代码可读性是项目经理衡量开发人员表现的最重要的因素。

至于成功的标准:工作软件和按时交付是最重要的，你可以在这里找到。

对于大多数项目经理来说，失败的首要原因是不切实际的期望，其次是评估和缺乏明确定义的可交付成果。

[![top delivery problems for software teams](img/a6e18dbd2e1e5c4042eab6b01562834a.png)T2】](https://apiumhub.com/wp-content/uploads/2018/07/Screen-Shot-2018-07-06-at-14.27.48.png)

_ 资料来源:*[_ 软件发展状况报告*](【http://codingsans.com/state-of-software-development-2018】T2)

而同行评审和 CI + TDD 是开发人员保证代码质量的首选。你可以通过其他方式在这里找到。

## 结论:全球软件发展形势

这篇文章只是我们从[调查](http://codingsans.com/state-of-software-development-2018)中收集的所有数据的一小部分。如果你想看完整的报告，只需点击这个链接下载。请随意与任何你认为可能感兴趣的人分享这篇文章或报告。但不要忘记，这些数据只与今年相关。很兴奋看到 2019 年会有什么不同。你有什么想法吗？

再次感谢 [Codingsans](https://codingsans.com/blog/software-development-trends-2018?msID=c04e1da2-6098-402f-b5f9-f5ec73da7f9c) 发起这一倡议，并与 [Instabug](https://instabug.com/) 、[离合器](https://clutch.co/)、[可发货](https://www.shippable.com/)、 [Codegiant](https://codegiant.io/home) 、 [Stride](https://www.stridenyc.com/) 、 [Codeship](https://codeship.com/) 、 [Usersnap](https://usersnap.com/) 、 [GitKraken](https://www.gitkraken.com/) 。

要查看完整报告，请点击[此处](http://codingsans.com/state-of-software-development-2018)。

尽情享受吧！

如果你想了解更多关于软件开发的有趣事实，我强烈推荐你点击[这里](http://eepurl.com/cC96MY)订阅我们的每月时事通讯。

## 如果你发现关于软件开发的有趣事实很有用，你可能会喜欢…

[Scala 泛型 I: Scala 类型界限](https://dev.to/apium_hub/scala-generics-i--scala-type-bounds-38)

[Scala generics II:协方差和逆变](https://dev.to/apium_hub/scala-generics-ii-covariance-and-contravariance-in-generics-5dib)

[Scala generics III:通用类型约束](https://dev.to/apium_hub/scala-generics-iii-generalized-type-constraints-58km)

BDD:用户界面测试

[Scala 中泛型类型的 F-bound](https://apiumhub.com/tech-blog-barcelona/f-bound-scala-generics/)

[微服务 vs 整体架构](https://apiumhub.com/tech-blog-barcelona/microservices-vs-monolithic-architecture/)

[“几乎无限”的可扩展性](https://apiumhub.com/tech-blog-barcelona/almost-infinite-scalability/)

[iOS Objective-C app:成功案例研究](https://dev.to/apium_hub/protected-ios-objective-c-app-cornerjob-successfull-case-study-89e)

[年度移动应用开发趋势](https://dev.to/apium_hub/mobile-app-development-trends-of-the-year)

[Banco Falabella 可穿戴设备案例研究](https://apiumhub.com/tech-blog-barcelona/banco-falabella-wearable-ios-android/)

[移动开发项目](https://apiumhub.com/software-projects-barcelona/)

[面向 iOS 应用的 Viper 架构优势](https://apiumhub.com/tech-blog-barcelona/viper-architecture/)

[为什么是科特林？](https://dev.to/apium_hub/why-kotlin-language-android-why-did-google-choose-kotlin--639)

[软件架构会议](https://dev.to/apium_hub/apiumhub-software-architecture-meetups-in-barcelona-31df)

[安卓纯 MVP](https://dev.to/apium_hub/pure-model-view-presenter-in-android-1736)

帖子[关于软件开发的有趣事实:统计 2018](https://apiumhub.com/tech-blog-barcelona/interesting-facts-software-development/) 首先出现在 [Apiumhub](https://apiumhub.com) 上。