# starter Project——一组最新的最佳实践被打包到一个个任务中
...
Starter Package 也是一个静态页面生成器，非常简单。最初的网站是使用 Starter Project 构建的，托管在 Netlify 上。它非常快，请看 [PageSpeed Insights](https://developers.google.com/speed/pagespeed/insights/?url=https%3A%2F%2Fstarter.silvestarbistrovic.from.hr%2F&tab=desktop) 和 [WebPageTest](https://www.webpagetest.org/result/180323_7F_8cd32d6a04d395ff01839c921d82047d/) 的分数。
...
您可以在`new_src`文件夹中找到所有资产。一旦您运行其中一个构建命令，`new_dist`文件夹将被创建，其中包含已编译的静态内容。点击了解更多关于结构[的信息。](https://starter.silvestarbistrovic.from.hr/structure/)
...
Starter 项目通过 babel 插件支持 ES6。你会发现延迟加载 CSS 文件的脚本(与关键 CSS 结合使用)和使用[关键 FOFT 方法](https://www.zachleat.com/web/comprehensive-webfonts/#critical-foft)加载字体的脚本。

如果您没有使用第三方服务来托管图像，那么 Starter Project 可以优化您的图像。Starter 项目支持
、`.gif`、`.png`、`.jpg`、`.svg`文件。

文档是每个项目不可或缺的一部分。您可以使用 Starter Project 生成优秀的文档。SassDoc 将解析 Sass 注释并创建 Sass 文档。 [JSDoc](http://usejsdoc.org/) 将解析 JavaScript 注释并创建 JavaScript 文档。你甚至可以创建生活方式指南。为此，Starter Project 使用 [KSS](https://kss-node.github.io/kss-node/) 来解析 Sass 注释。
...
*   韩网站 -一个为来自加拿大蒙特利尔的设计师兼 FX 技术总监韩提供博客的网站。这个网站是通过中间人运作的。
*   Irina & Matej 网站-Irina 和 Matej 的网站，他们是克罗地亚的婚礼和肖像摄影师。该网站在 WordPress 上运行。
*   Silvestar Bistrovi 网站 -一个拥有博客的个人网站。该网站在 Hexo 上运行。
*   [首发项目网站](https://starter.silvestarbistrovic.from.hr/) -首发项目的网站。该网站仅在启动项目上运行。
