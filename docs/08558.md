...
> 原文:[https://dev . to/nejckorasa/open-source-insta gram-analyzer-1 OBD](https://dev.to/nejckorasa/open-source-instagram-analyzer-1obd)
...
### [](#store-all-instagram-media-data)📍存储所有 instagram 媒体数据📷

应用程序加载所有用户的 instagram 媒体，并以 JSON 格式保存。该数据包括所有媒体元数据，包括喜欢、位置、标记的用户、评论、图像 url...

### [](#store-all-instagram-location-data)📍存储所有 instagram 位置数据📊

分析地理标签并将位置保存在 JSON forma 中。这些数据包括每个地点的事件以及图片和 instagram 媒体 url...

### [](#store-all-instagram-countries-and-cities-location-data)📍存储所有 instagram 国家和城市的位置数据

国家和城市还通过使用 [LocationIQ API](https://locationiq.com) 进行反向地理编码进行分析。数据保存在 JSON 文件中。

### [](#prints-occurrences-for-location-country-and-city)📍打印✈️地点，国家和城市事件
...
### [](#print-table-view-of-most-visited-location-countries-and-cities)打印最常去的地点、国家和城市的表格视图🌍
...
## [](#configuration-amp-options)配置&选项

### [](#acquire-tokens)获取代币

##### [](#acquire-instagram-access-token)获取 Instagram 访问令牌

去 [Pixelunion](http://instagram.pixelunion.net/) ，生成令牌，别忘了令牌！

##### [](#acquire-location-iq-access-token)获取位置 IQ 访问令牌

去[地点智商](https://locationiq.com/)，报名，领令牌，别忘了令牌！

### [](#configure-and-run)配置并运行
...
#### [](#why-does-it-take-so-long-to-load-additional-location-data)为什么加载附加位置数据需要这么长时间？

对于反向地理编码，使用 Location IQ API。API si 免费版本限制为每秒 1 个请求。这就是为什么额外的数据加载需要花费`<different_location_count>`秒。

## [](#go-check-it-out-leave-feedback)去看看，留下反馈🙏
