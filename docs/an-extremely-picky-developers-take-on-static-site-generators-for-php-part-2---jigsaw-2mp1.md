...
让我们看一下刚刚生成的默认目录结构。在顶级目录中，我们有一个`config.php`,代码如下:
...
我们也有 Javascript 依赖。我看到一个`webpack.mix.js`，它让我知道 [Laravel Mix](https://laravel.com/docs/5.6/mix) 正在被使用。所以我运行`npm install`,与此同时，我将浏览文档，试图更多地理解我正在做的事情😉。
...
Laravel Mix 提供了对 Sass 的现成支持，所以`source/_assets/sass/main.scss`是您的起点。SCSS 可能不是 CSS 领域最新最闪亮的东西，但它经过了战斗的考验，肯定能完成任务，所以我赞成它的加入。让我们把这一页变成能引起你注意的东西
...
我想建立一个基本的博客，用 Markdown 文件写文章。为了做到这一点，让我们引入[集合](http://jigsaw.tighten.co/docs/collections/)的概念，正如文件所述，*让您能够在聚合级别*访问您的内容。听起来很适合写博客。还记得之前的`config.php`文件吗？让我们调整一下，告诉 Jigsaw 我们收集的博客帖子:
...
让我们将这个文件保存为`the-fresh-prince.md`，然后按照前面的`extends`部分指定的那样创建适当的模板:`source/_layouts/post.blade.php`。
...
好的，看起来相当容易。作为额外的帮助，在谷歌搜索之后，我找到了一篇关于用 Jigsaw 创建网站地图的[文章](https://mattstauffer.com/blog/adding-an-auto-generated-sitemap-to-your-jigsaw-based-static-site/)，以及事件特性的原始[拉请求](https://github.com/tightenco/jigsaw/pull/189)，所以我有足够的信息来解决这个问题。让我们一起来破解这个东西吧！
...
*   如果检测到`local`环境，整个过程将停止。这样做是为了避免性能影响，因为每次构建时都必须从 Contentful 获取所有数据(如果在监视模式下运行，这可能非常频繁)。这个想法是首先运行一个`./vendor/bin/jigsaw build production`来获取所有内容，然后对本地存储的内容进行操作。当然，一个好主意是将生成的文件添加到本地`.gitignore`，以避免将它们保留在版本控制中(内容不属于那里)。
...
为了解决这个问题，Matt 向我透露，他们正在用网站框架构建一个生成器，以解决最常见的需求(如博客、文档网站等)，并用 [Github issue](https://github.com/tightenco/jigsaw/issues/236) 来跟踪进度。他还提到他们刚刚推出了一个新的用 Jigsaw 构建的网站的[展示，我鼓励你去看看。](https://builtwithjigsaw.com/)
...
下一篇文章将是关于 [Couscous](http://couscous.io/) 的，它将自己描述为一个文档生成器。敬请期待！