...
> 原文:[https://dev.to/adarosecannon/isomorphic-esmodules-ko](https://dev.to/adarosecannon/isomorphic-esmodules-ko)

## [](#aka-one-weird-trick-to-share-nodemodules-with-browser-side-scripts)又名，一个与浏览器端脚本共享 node_modules 的奇怪技巧。
...
### [](#defining-the-terms-es-modules-and-isomorphic)定义术语 ES 模块和同构
...
### [](#using-es-modules-to-write-a-single-page-application-with-server-side-rendering-for-the-first-load)使用 ES 模块编写单页应用程序，服务器端渲染为第一次加载。
...
### [](#1-setup-client-side-rendering)1。设置客户端渲染
...
### [](#2-responding-to-click-events)2。响应点击事件。
...
### [](#3-using-es-modules-in-the-node-server)3。在节点服务器中使用 ES 模块
...
### [](#4-using-the-templates-on-the-server)4。使用服务器上的模板
