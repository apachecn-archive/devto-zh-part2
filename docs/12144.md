...
## [](#2-send-a-lost-stash-back-where-it-comes-from)2。把丢失的东西送回原处
...
### [](#1-i-still-cant-see-my-recovered-stash)1。我还是看不到我找回的东西
...
### [](#2-my-git-isnt-in-english)2。我的 Git 不是英语
...
### [](#commit-messages-are-healthy)提交消息正常
...
### [](#yes-raw-git-stash-apply-endraw-gt-raw-git-stash-pop-endraw-)是的，`git stash apply` > `git stash pop`

与`git stash pop`， **`git stash apply`** 不同的是，`git stash apply` 并不从藏物列表中移除藏物，可以避免一些损失。

### [](#branches-gt-stashes)树枝>藏东西
