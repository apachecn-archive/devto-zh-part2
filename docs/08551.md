...
> 原文:[https://dev.to/dceddia/redux-vs-the-react-context-api-1nof](https://dev.to/dceddia/redux-vs-the-react-context-api-1nof)
...
## [](#a-motivating-example)一个激励人心的例子
...
## [](#using-redux-to-improve-data-flow)使用 Redux 改善数据流
...
### [](#-raw-connect-endraw-is-pure)`connect`纯粹
...
### [](#easy-debugging-with-redux)用 Redux 轻松调试
...
### [](#customize-redux-with-middleware)用中间件定制 Redux
...
## [](#how-to-use-the-react-context-api)如何使用 React 上下文 API
...
### [](#consumer-uses-the-render-props-pattern)消费者使用渲染道具模式
...
### [](#provider-accepts-one-value)提供者接受一个值

只有一个值，作为`value`道具。但是请记住，该值可以是任何值。实际上，如果你想向下传递多个值，你可以创建一个包含所有值的对象，然后向下传递对象。

这是上下文 API 的基本内容。

## [](#context-api-is-flexible)上下文 API 灵活

因为创建上下文给了我们两个组件(提供者和消费者)，所以我们可以随意使用它们。这里有几个想法。

### [](#turn-the-consumer-into-a-higherorder-component)把消费者变成更高阶的成分
...
### [](#hold-state-in-the-provider)保持提供者中的状态
...
### [](#pass-actions-down-through-context)通过上下文向下传递动作
...
## [](#should-you-use-context-or-redux)应该用上下文，还是 Redux？
