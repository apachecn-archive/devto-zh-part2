# 将您的安全 Vue.js 应用程序部署到 AWS

> 原文：<https://dev.to/oktadev/deploy-your-secure-vuejs-app-to-aws-5g5>
...
修改`./src/router/index.js`以匹配下面的代码片段:
...
*   windows:[https://aws.amazon.com/cli/](https://aws.amazon.com/cli/)
...
Okta 有一个方便的 Vue 组件来处理与他们的服务集成的所有繁重工作。要安装 Okta Vue SDK，运行以下命令:
...
打开`src/router/index.js`并将其修改成如下代码。另外，一定要把`{yourClientId}`和`{yourOktaDomain}`改成你的！
...
接下来是创建一个定义应用程序的文件。将以下代码复制到`app.js`中，并将`{yourClientId}`和`{yourOktaDomain}`改为您的代码。
...
*   [用 Vue.js 和节点](https://dev.to/blog/2018/02/15/build-crud-app-vuejs-node)构建一个基本的 CRUD App
*   [在 20 分钟内为您的普通 JavaScript 应用添加身份验证](https://dev.to/blog/2018/06/05/authentication-vanilla-js)
*   [在 15 分钟内构建带有用户认证的 React 应用](https://dev.to/blog/2017/03/30/react-okta-sign-in-widget)
*   [用 Okta 的签到工具在 15 分钟内创建一个 Angular 应用](https://dev.to/blog/2017/03/27/angular-okta-sign-in-widget)
