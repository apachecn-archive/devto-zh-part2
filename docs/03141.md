# 7+互联网最佳数据可视化课程【2019 更新】

> 原文:[https://dev . to/sinx loud/data-visualization-course-in-tableau-python-d3js-2jl 6](https://dev.to/sinxloud/data-visualization-course-in-tableau-python-d3js-2jl6)

在这篇文章中，我将简要介绍一些最好的数据可视化课程，这些课程将帮助你发展行业现成的数据可视化技能。

所有这些课程都可以在网上获得，其中一些费用很低，将帮助您学习并擅长使用 Python、Tableau、d3.js 和其他程序或工具进行数据可视化。这些课程适合初学者和中级学习者。

## [](#best-data-visualization-courses-and-specialization-2018)最佳数据可视化课程和专业🤞[2018]

这篇文章是最初发表在这里的文章的一部分> >[数据可视化课程](https://sinxloud.com/where-learn-find-best-data-visualization-courses/)。

* * *

### [](#1-data-visualization-with-tableau-specialization-coursera)1。[数据可视化与 Tableau 专业化](https://sinxloud.com/fly/data-visualization-with-tableau-coursera/) (Coursera)

本课程由加州大学戴维斯分校创建。在本课程“数据可视化”中，您将通过了解什么是数据可视化，以及如何使用它来更好地查看和理解数据，来学习使用 Tableau 可视化业务数据。

您将研究数据可视化的基本概念，探索 Tableau 接口，识别和应用 Tableau 提供的各种工具。

本课程结束时，您将能够准备数据并将数据导入 Tableau，并解释数据分析和数据可视化之间的关系。

[![Data Visualization with Tableau Specialization](../Images/c79215dcc14fd79b7bb3b5e3e92771cf.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--kEKM7A_D--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://sinxloud.com/wp-content/uploads/2018/08/Data-Visualization-with-Tableau-Specialization-Coursera-1024x371.png.webp)

#### [](#what-youll-learn)你会学到什么？

*   Tableau 可视化基础
*   Tableau 的基本设计原则
*   使用 Tableau 的可视化分析
*   创建仪表板和用 Tableau 讲故事
*   Tableau 项目的数据可视化

你可以在这里注册🔗T3】

* * *

### [](#2-data-visualization-a-practical-approach-for-absolute-beginners-edx)2。[数据可视化:绝对初学者的实用方法](https://sinxloud.com/fly/data-visualization-a-practical-approach-for-absolute-beginners-edx/) (edX)

本课程由微软创建，发布在 edX 上。在本课程“面向初学者的数据可视化”中，您将了解实用的数据可视化技术、提示和技巧，这些技术、提示和技巧适用于有抱负的分析师以及希望在任何需要这一 21 世纪关键技能的领域从头开始的人。

你可以免费学习本课程的所有四个模块，在课程结束时，你将成为这门艺术的实践者。

[![Data Visualization: A Practical Approach for Absolute Beginners ](../Images/bf1f7bc0a9e5b9fe11402df5ed073e32.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--KZGClmfT--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://sinxloud.com/wp-content/uploads/2018/08/Data-Visualization-A-Practical-Approach-for-Absolute-Beginners-by-Microsoft-edX-1-1024x359.png.webp)

#### [](#what-youll-learn)你会学到什么？

*   视觉素养:构建最常用和最简单的数据可视化和图表类型
*   可视化分析:理解交互、设计和可视化科学如何增强基本的数据可视化工件
*   业务可视化:为工作和生活中的应用程序创建基本报告和仪表盘
*   明天的可视化:学习如何避免常见的陷阱，并使可视化工作。

你可以在这里注册🔗T3】

* * *

### [](#3-data-visualization-using-tableau-public-pluralsight)3。[数据可视化使用 Tableau Public](https://sinxloud.com/fly/data-visualization-using-tableau-public-pluralsight/) (Pluralsight)

本课程由 Robert Spent 创建，他在 Microsoft 的 Team Foundation Server 管理和版本控制团队中工作了近 10 年。

本课程在 Pluralsight 上的评分为 4.8 星，将帮助您掌握使用 Tableau Public 构建数据可视化技能。

[![Data Visualization Using Tableau Public](../Images/054c0ddbb39e3092dc337266cb640e43.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--uab-yOX9--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://sinxloud.com/wp-content/uploads/2018/08/Data-Visualization-with-Python-Coursera-1024x309.png.webp)

#### [](#what-youll-learn)你会学到什么？

*   创建您的第一个仪表板
*   绘制消费者支出图表
*   获胜的代价

你可以在这里注册🔗T3】

* * *

### [](#4-data-visualization-with-python-coursera)4。[用 Python 实现数据可视化](https://sinxloud.com/fly/data-visualization-with-python-ibm-coursera/) (Coursera)

该课程由 IBM 创建，在 Coursera 上拥有 4.6 星用户评级。在本课程“Python 数据可视化”中，您将学习如何获取乍看之下没有什么意义的数据，并以简单易懂的形式呈现这些数据。

您将学习各种已经开发出来的可视化显示数据的技术，并使用 Python 中的几个数据可视化库，如 Matplotlib、Seaborn 和 Folium。

[![Data Visualization with Python](../Images/054c0ddbb39e3092dc337266cb640e43.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--uab-yOX9--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://sinxloud.com/wp-content/uploads/2018/08/Data-Visualization-with-Python-Coursera-1024x309.png.webp)

#### [](#what-youll-learn)你会学到什么？

*   数据可视化工具简介
*   最佳和专业的可视化工具
*   高级可视化和地理空间数据

你可以在这里注册🔗T3】

* * *

### [](#5-introduction-to-data-visualization-with-python-datacamp)5。[Python 数据可视化简介](https://www.datacamp.com?tap_a=5644-dce66f&tap_s=384177-0102f2) (DataCamp)

本课程由 DataCamp 讲师达维德·阿鲁利亚(数据科学家和应用数学家)和布莱恩·范·德·Ven(Anaconda 软件工程师和 Bokeh 开发者)创建。

本课程涵盖的主题包括定制图形、绘制二维阵列(例如伪彩色图、等高线图、图像等)。)、统计图形(例如，可视化分布和回归)，以及处理时间序列和图像数据。

[![Introduction to Data Visualization with Python](../Images/ed17d9f09ee7c5b771be94dd526ccce3.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--Uqk_MMY7--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://sinxloud.com/wp-content/uploads/2018/08/Introduction-to-Data-Visualization-with-Python--1024x356.png.webp)

#### [](#what-youll-learn)你会学到什么？

*   自定义地块
*   绘制 2D 阵列
*   Seaborn 的统计图
*   分析时间序列和图像

**你可以[在这里报名](https://www.datacamp.com?tap_a=5644-dce66f&tap_s=384177-0102f2)**

* * *

### [](#6-data-visualization-for-developers-pluralsight)6。[面向开发者的数据可视化](https://sinxloud.com/fly/data-visualisation-for-developers-pluralsight/) (Pluralsight)

本课程由 Dan Appleman 创建，他是 Full Circle Insights 的首席技术官，也是一位著名的公共演说家和作家。面向开发人员的数据可视化课程在 Pluralsight 上获得 4.2 星评级，将向任何开发人员介绍数据可视化的各个方面，从故事到数字，到架构，再到代码。

本课程涵盖的主题包括数据可视化的基础知识，无论是架构、平台、工具还是语言，每个开发人员都需要了解这些知识。

[![Data Visualization for Developers](../Images/c7961879b4688dde3eee4417e622fd58.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--dM4Qx0xl--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://sinxloud.com/wp-content/uploads/2018/08/Data-Visualization-for-Developers-Pluralsight-1024x391.png.webp)

#### [](#what-youll-learn)你会学到什么？

*   数据可视化简介
*   制图-1 A
*   商业智能
*   可视化建筑
*   亲身体验可视化

你可以在这里注册🔗T3】

* * *

### [](#7-interactive-data-visualization-with-bokeh-datacamp)7。[使用散景的交互式数据可视化](https://www.datacamp.com?tap_a=5644-dce66f&tap_s=384177-0102f2)(数据营)

交互式数据可视化课程是掌握 Bokeh 的完整指南，Bokeh 是用于构建高级和现代数据可视化 web 应用程序的 Python 库。由 Anaconda 的软件工程师和 Bokeh 的开发者布莱恩·范·德 Van 创建。在本课程中，您将学习如何利用 Python(和其他语言)的散景可视化库，该库面向现代 web 浏览器进行演示，并通过简单的命令快速构建复杂的统计图。

[![Interactive Data Visualization with Bokeh](../Images/983c4ea42c80353932f68bef3ae2b92e.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--FH3YgKXO--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://sinxloud.com/wp-content/uploads/2018/09/Interactive-Data-Visualization-with-Bokeh-1024x421.png.webp)

#### [](#what-youll-learn)你会学到什么？

*   对使用散景进行基本绘图及其用途的深入介绍。
*   将多个散景图合并到布局中，将图链接在一起，使用图例和悬停工具提示进行注释。
*   用 Bokeh 构建互动应用，集成 Numpy 和 Pandas。
*   在绘图时使用 Bokeh 内置部件，并使用 Bokeh 服务器连接到 python 代码。
*   基于著名的 Gapminder 数据集，从头开始构建复杂的散景数据探索应用程序。

你可以在这里注册🔗T3】

* * *

### [](#8-d3js-data-visualization-fundamentals-pluralsight)8。 [D3.js 数据可视化基础](https://sinxloud.com/fly/data-visualisation-for-developers-pluralsight/) (Pluralsight)

Pluralsight 徽标

本课程由 Ben Sullins 创建，他是一名认证的数据极客和作者，拥有 15 年的行业经验。本课程在 Pluralsight 上的评分为 4.8 星，它将帮助您学习如何使用 D3.js(数据驱动文档)实现强大的数据可视化。

本课程主要集中在一旦发现一个洞见，如何可视化地编码和呈现数据给观众。

[![D3.js Data Visualization Fundamentals](../Images/3385a1081a50afa59da6c01f43168fc8.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--5LP89E_1--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://sinxloud.com/wp-content/uploads/2018/08/D3.js-Data-Visualization-Fundamentals-Pluralsight-1024x354.png.webp)

#### [](#what-youll-learn)你会学到什么？

*   基础知识和图表
*   使用数据外部数据源
*   使用刻度和轴增强您的视觉效果
*   通过交互性增强您的视觉效果
*   绘制地图并把它们放在一起

你可以在这里注册🔗T3】

* * *

* * *

##### [](#before-you-go)**出发前🍻**

如果你喜欢这篇文章，请分享给你的朋友，并关注这个 [Dev.to](https://dev.to/sak1m) Handle，每两周更新一次类似的内容。

祝你事业顺利！干杯！

###### [](#happy-learning-)快乐学习！！！