...
> 原文:[https://dev . to/pongsatt/setup-kubernetes-cluster-using-kube ADM-hpb](https://dev.to/pongsatt/setup-kubernetes-cluster-using-kubeadm-hpb)
...
# [](#switch-to-root-user-all-machines)切换到 root 用户(所有机器)
...
# [](#install-docker-all-machines)安装 Docker(所有机器)

按照以下步骤将 docker 安装到所有机器上。

### [](#1-install-docker)1。安装 Docker
...
### [](#2-configure-docker)2。配置 docker
...
### [](#3-optional-install-image-cleanup-job)3。(可选)安装映像清理作业
...
### [](#4-check-if-everything-alright)4。检查是否一切正常
...
# [](#install-kubeadm-all-machines)安装 Kubeadm(所有机器)

下一步是安装 kubernetes 集群工具。

### [](#1-disable-swap)1。禁用交换
...
### [](#2-run-below-commands-on-all-machines)2。在所有计算机上运行以下命令
...
# [](#setup-cluster-master-node-only)设置集群(仅限主节点)
...
# [](#setup-cluster-worker-node-only)设置集群(仅限工作节点)
...
# [](#post-install-master-node-only)安装后(仅主节点)

### [](#1-setup-cluster-connection-configuration)1。设置群集连接配置
...
### [](#2-install-pod-network-weavenet)2。安装 Pod 网络(weavenet)
...
# [](#install-dashboard-optional)安装仪表板(可选)

为了能够看到集群的全貌，您需要一个仪表板。

### [](#1-install-dashboard)1。安装仪表板
...
### [](#2-setup-permission-to-access-dashboard)2。设置访问仪表板的权限
...
### [](#4-open-dashboard-ui)4。打开仪表板 UI
