...
*   使用 DOM API(大部分)很难。React 基本上让开发人员能够使用比真实浏览器更友好的虚拟浏览器。
*   React 只是 JavaScript。有一个非常小的 API 需要学习，之后你的 JavaScript 技能会让你成为一个更好的 React 开发人员。没有进入的障碍。
*   学习 React 也为 iOS 和 Android 移动应用程序带来了巨大的回报。React Native 允许您使用 React 技能构建本地移动应用程序。您甚至可以在您的 web、iOS 和 Android 应用程序之间共享一些逻辑。
*   facebook.com 脸书的 React 团队测试 React 引入的所有改进和新特性，这增加了社区对该库的信任。在 React 版本中很少看到大而严重的错误，因为它们只有在脸书经过全面的生产测试后才会发布。
*   最重要的是，React 使开发人员能够以声明的方式描述他们的用户界面，并对这些界面的状态进行建模。这意味着开发人员不用想出步骤来描述接口上的事务，而只是根据最终状态(像函数一样)来描述接口。当事务发生在那个状态时，React 负责根据那个状态更新用户界面。

* * *

让我们进入更多的细节。React 的一个著名优势是它的**虚拟 DOM** (或协调算法)。我们将通过一个例子来展示拥有这样一个算法的实际价值。
...
我们将从头开始这个例子。创建一个新目录，并在那里启动你最喜欢的编辑器:
...
在该目录中创建一个`index.html`文件，并在其中放置一个标准的 HTML 模板。在模板中包含一个`script.js`文件，并在脚本中放入一个`console.log`语句，以测试包含文件是否工作:
...
在您的浏览器中打开`index.html`文件，确保您可以看到没有问题的空模板，并且您可以在控制台开发工具选项卡中看到您放入`script.js` :
中的`console.log`测试消息
...
现在，让我们引入 React 库本身，我们可以从 [Reactjs 网站](https://reactjs.org/docs/cdn-links.html)中包含它。复制`react`和`react-dom`脚本，并包含在`index.html` :
中
...
现在在`script.js`中，让我们通过 id 获取这个新的`div`元素，并将其放入一个常量中。让我们把这个常数命名为`jsContainer`。我们可以用`document.getElementById`从 HTML:
中抓取`div`
...
为了控制这个`div`的内容，我们可以直接在`div`元素上使用`innerHTML` setter 调用。我们可以使用这个调用来提供任何想要插入到 DOM 中的 HTML 模板。让我们插入一个`div`元素，它有一个类“demo”和字符串“Hello JS”作为它的内容:
...
识别出 react 容器后，我们现在可以使用 ReactDOM 库来`render`将 HTML 模板的版本反应到这个容器:
...
这里是我们目前为止使用 React 的等效 HTML 用户界面:
...
我们现在可以测试这个。浏览器应该同时呈现“Hello JS”和“Hello React”。让我们使用这个 CSS 将演示 divs 设计成一个盒子，这样我们就可以直观地分割屏幕。在`index.html` :
...
在我们的 HTML 模板中嵌套元素，在 JS 版本中很简单，因为它只是 HTML。例如，要让演示程序`div`呈现一个`<input />`元素，我们只需将它添加到内容:
...
为了在 React 中做同样的事情，我们向顶层的`div`元素添加了第五个参数。这个新的第五个参数是另一个`React.createElement`调用，这次使用了一个`p`标记，没有属性，而`new Date()`字符串表示内容:
...
*本文原载[此处](https://medium.freecodecamp.org/yes-react-is-taking-over-front-end-development-the-question-is-why-40837af8ab76)T3】*

* * *

学习 React 还是 Node？检查我的书；)

*   [通过构建游戏学习 react . js](https://jscomplete.com/learn-react-js-by-building-games)
*   [Node.js 超越基础](https://jscomplete.com/learn-node-js-beyond-the-basics)

我为[的](https://app.pluralsight.com/profile/author/samer-buna)、[的](https://www.lynda.com/Samer-Buna/7060467-1.html)和[的](https://jscomplete.com/samer-buna)创建了**在线课程**。我还为团队做**在线和现场培训**，涵盖 JavaScript、Node.js、React.js 和 GraphQL 的初级到高级水平。如果你想为你的团队预约会议，请发电子邮件给 training@agilelabs.com。