...
> 原文:[https://dev . to/kball/Friday-frontend-check-out-the-vue-edition-26f 0](https://dev.to/kball/friday-frontend-check-out-the-vue-edition-26f0)
...
##### [](#css-borderradius-can-do-that)[CSS 能做到吗？](https://9elements.com/io/css-border-radius/)

我把标题全部大写，因为这让我大吃一惊。我经常使用边框半径，但通常都设置相同的半径。我有一个模糊的想法，你可以独立设置不同角落的半径，但我不知道你可以独立设置垂直和水平半径…这里有这么多令人惊讶的可能性。

##### [](#css-previous-sibling-selectors-and-how-to-fake-them)[CSS 以前的兄弟选择器以及如何伪造它们](https://medium.freecodecamp.org/how-to-make-the-impossible-possible-in-css-with-a-little-creativity-bd96bb42b29d)

这是一个很好的练习，探索了兄弟(+)和后续兄弟(~)选择器，以及如何使用它们来伪装“上一个兄弟”选择器。即使你不想这样做，这也会给你一些很好的实践，让你思考这些兄弟选择器的应用。

##### [](#valid-css-content)[有效 CSS 内容](https://css-tricks.com/valid-css-content/)

我一直在使用 content 属性，因为我用 before/after 伪元素做了很多事情，它们需要设置内容才能显示。但是我从来没有想过要探索用内容呈现的可能性的界限…这真的很疯狂。

##### [](#negative-grid-lines)[负网格线](https://css-irl.info/negative-grid-lines/)

我使用 CSS Grid 的次数还不够多，还没有把它融入到我的心理模型中，所以这花了我大约 3 次通读和一些戳在演示中，但后来它让我大吃一惊。

#### [](#javascript)JavaScript

##### [](#how-to-import-a-sass-file-into-every-vue-component-in-an-app)[如何将 Sass 文件导入到 App 中的每个 Vue 组件](https://css-tricks.com/how-to-import-a-sass-file-into-every-vue-component-in-an-app/)

这太棒了。使用 Vue 的单个文件组件方法可以让你隔离每个组件的风格，但是正如文章所说——许多设计属性是完全全球化的，因为内聚性在设计中是很重要的。没问题，如果这些属性被封装在文件中，那么将它们自动导入到 Vue 组件中是非常简单的。太棒了。

##### [](#advanced-vue-debugging-a-detective-story)[高级 Vue 调试:侦探故事](https://vuejsdevelopers.com/2018/10/16/vue-js-advanced-debugging/)
...
##### [](#what-you-need-to-know-about-vue-cli-3)[关于 Vue CLI 3 你需要知道的事情](https://dev.to/progresstelerik/what-you-need-to-know-about-vue-cli-3-56h5-temp-slug-3068326)
...
##### [](#lazy-loading-images-with-vuejs-directives-and-intersection-observer)[用 Vue.js 指令和交集观察器懒加载图片](https://css-tricks.com/lazy-loading-images-with-vue-js-directives-and-intersection-observer/)
...
##### [](#accessibility-cheatsheet)[可访问性备忘单](https://moritzgiessmann.de/accessibility-cheatsheet/)
...
##### [](#introducing-github-actions)[介绍 GitHub 动作](https://css-tricks.com/introducing-github-actions/)

虽然技术上与前端无关，但这似乎足够大，值得强调。Github 刚刚宣布能够设置本质上定制的 IFTTT 风格的代码触发器，作为平台上发生的事情的结果。这为一些更加强大的持续集成解决方案以及一些其他非常有趣的可能性打开了大门。

##### [](#designing-for-cognitive-differences)[为认知差异而设计](https://alistapart.com/article/designing-for-cognitive-differences)

有趣的是，看看你的设计和实现选择如何影响你的网站/应用程序如何被不同形式的智力挑战的人使用。我发现关于焦虑的部分特别有帮助(可能是因为我知道很多人都在焦虑中挣扎)，但它们都很好。

##### [](#start-performance-budgeting)[开始绩效预算](https://addyosmani.com/blog/performance-budgets/)
