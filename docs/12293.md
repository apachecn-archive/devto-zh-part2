...
> 原文:[https://dev . to/bocaynotnef/receiving-data-from-esp8266-sensors-3n5e](https://dev.to/bocajnotnef/receiving-data-from-esp8266-sensors-3n5e)
...
# [](#step-one-get-the-data-off-the-network)第一步:从网络上获取数据

现在，让我们假设我们只期待一个客户端传感器与我们对话。我们稍后将处理多个传感器，但之后我们必须处理线程和锁定，这是一个巨大的混乱。

所以现在，一个传感器。

## [](#aside-server-socket-programming)暂且不提:服务器套接字编程
...
# [](#step-two-threading-the-connections)第二步:穿线连接
...
## [](#accepting-incoming-connections)接受呼入连接
...
## [](#listening-to-those-incoming-connections)监听那些呼入的连接
...
## [](#interacting-with-a-database)与数据库交互
...
## [](#thats-great-but-what-does-the-main-look-like)太好了，但是主长什么样？
