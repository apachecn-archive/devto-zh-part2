# Coursehunt:免费搜索你的课程！

> 原文：<https://dev.to/theiyd/coursehunt-2g8k>

[![](img/628ce579ed9d3f6b0e13ce19056b3017.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--FEvkWaan--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/llonxno0e2pi13dcz2uy.png)

## 课程搜索

一个用电子制作和构建的桌面应用程序，让你能够批量下载由 [coursehunters](coursehunters.net) 提供的免费课程。现在，Coursehunters 是一个提供免费在线课程下载的平台，实际上是在全球传播知识。

桌面应用在 [Github](https://github.com/theIYD/coursehunt) 上完全开源，可用于 Windows。

***注:**该应用将在未来版本中提供给其他平台。*

### 表现出一些对❤️的喜爱

如果你喜欢这个项目，在资源库上留下一颗星星，这样可以帮助这个项目越来越成长。

### 投稿🙌

随意派生项目，并打开拉式请求。

编码快乐！