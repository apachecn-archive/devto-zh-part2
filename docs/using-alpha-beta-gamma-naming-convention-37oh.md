...
过了一段时间，我开始对 Sass 变量使用同样的方法。这是我的网站的颜色变量的样子:
...
使用 Alpha，Beta，Gamma 命名约定的好处在于它将上下文从元素中分离出来。此外，[希腊字母]被广泛使用，所以你已经知道一些字符的几率相当高。
...
> 使用`psi`和`omega`来命名不同或完全不同的变量或修饰符。

给你一个提示:使用`psi`和`omega`字母(字母表的最后两个字母)来命名不同的或完全不同的变量或修饰语。例如，我用`color-psi`表示文字颜色，用`color-omega`表示白色。这些是`color-alpha,` `color-beta,`和`color-gamma`的反义词，在我的例子中它们是主题/品牌颜色。
