...
> 原文:[https://dev.to/thobyv/hyperapp-hyperapp-路由器-创建电影-购物车-网络应用-47d](https://dev.to/thobyv/hyperapp--hyperapp-router-create-a-movie-shopping-cart-web-app-47d)
...
#### [](#-raw-src-folders-endraw-)`/src folders:`
...
#### [](#-raw-src-files-endraw-)`/src files:`
...
## [](#now-lets-get-to-it)现在言归正传！

### [](#set-up-our-app-state-model)建立我们的 app 状态模型。
...
### [](#mutating-our-state-data-using-actions)使用动作变异我们的状态数据。
...
#### [](#-raw-add-movie-to-cart-endraw-action)`Add movie to cart`动作:
...
#### [](#-raw-filter-movies-by-price-range-and-shipping-endraw-actions)`Filter movies by price range and shipping`动作:
...
## [](#rendering-our-state-dataexecuting-actions-using-hyperapp-view-function-components)使用 hyperapp *视图*功能组件渲染我们的状态数据/执行动作。
...
#### [](#-raw-ltappgt-endraw-component)`<App/>`分量。
...
#### [](#-raw-ltmoviecardgt-endraw-component)`<MovieCard/>`分量。
...
#### [](#-raw-ltviewmoviedetailsgt-endraw-component)`<ViewMovieDetails/>`分量。
...
#### [](#-raw-ltmoviedetailsgt-endraw-component)`<MovieDetails/>`分量。
...
#### [](#-raw-ltviewcartgt-endraw-component)`<ViewCart/>`分量。
...
#### [](#-raw-ltcartitemsgt-endraw-component)`<CartItems/>`分量。
...
#### [](#-raw-ltnavbargt-endraw-component)`<NavBar/>`分量。
...
### [](#registering-our-lazy-components-to-their-respective-route-paths)将我们的惰性组件注册到它们各自的路由路径。
