...
> 原文:[https://dev.to/sait/play-with-the-react-router-1e79](https://dev.to/sait/play-with-the-react-router-1e79)

玩反应路由器游戏。

# [](#what-is-a-react-router)什么是 React 路由器？

React 路由器帮助我们在 React 应用程序中进行路由，React 路由器中的一切都是基于组件的。

#### [](#is-it-dynamic-or-static-routing)是动态路由还是静态路由？
...
##### [](#there-is-an-object-available-in-our-component)我们的组件里有一个可用的对象。

## [](#who-gives-us-this-object)谁给了我们这个对象？

答案是，每当我们为组件声明路由时，React router 都会将对象传递给组件。

例如，我们为 App 组件和 Posts 组件定义了 routes，这样这些组件现在就知道上图中的这些对象了。

## [](#programmatically-navigate-with-react-router)用 React 路由器编程导航。
...
### [](#play-with-react-router-part2)[玩 React 路由器 Part-2](https://dev.to/saigowthamr/play-with-react-router-part-2-4i8m)
