...
> 原文:[https://dev.to/timber/the-pythonic-guide-to-logging-4p35](https://dev.to/timber/the-pythonic-guide-to-logging-4p35)
...
### [](#printwhy-not-just-use-the-print-statement)print('为什么不直接用 print 语句？')
...
#### [](#wrap-expensive-calls-in-a-level-check)将昂贵的通话包在等级检查中
...
#### [](#avoid-logging-in-the-hot-path)避免登录热路径

热路径是对性能至关重要的代码，因此它经常被执行。最好避免在这里记录日志，因为这会成为 IO 瓶颈，除非因为要记录的数据在热路径之外不可用而必须这样做。

## [](#storing-amp-accessing-these-logs)存储&访问这些日志
