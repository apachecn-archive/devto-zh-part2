...
> 原文：<https://dev.to/hoverbaum/polling-using-rxjs-3c3i>
...
你应该对 Streams / [Observables](http://reactivex.io/documentation/observable.html) 有一个基本的了解，并且有扎实的 JavaScript 基础才能享受这篇文章。在这篇文章的其余部分，我将把 Stream 和 Observable 视为同一事物的可互换词。虽然我们将涵盖许多基本的东西，但它们主要是 RxJS 细节，而不是关于流的基础知识。如果你正在寻找一个一般性的介绍，考虑一下主旨标题“[你错过的反应式编程介绍](https://gist.github.com/staltz/868e7e9bc2a7b8c1f754)”。
...
一旦我们开始轮询，我们希望每秒两次获得该任务的当前状态，并停止轮询一次`processing === false`。
...
首先，我们需要每隔 *x* 时间发出一个事件。RxJS 为此提供了两个功能:

*   间隔

*   计时器

*间隔*在给定时间后发出第一个事件，然后以相同的间隔连续发出，而*计时器*在给定时间后启动，每隔 *x* 时间发出一次事件。对于每秒两次更新，我们可以从使用 timer(0，500)开始。这将从蝙蝠的右边开始触发事件，之后每秒两次。
...
**。管道**是 RxJS 指定转换将在流上发生的方式。通过将操作符提取到它们自己的导入中，RxJS 实现了比重载的 Observable 实现更好的树摇动，更多上下文参见[的解释](https://github.com/ReactiveX/rxjs/blob/master/doc/pipeable-operators.md)。

> # pipeline is the wrapper of RxJS corresponding to the transformation of event stream.

这样做的结果将是流的流**。每个发射值本身都是可观测的。为了管理混乱，我们可以通过 *concatMap* 来处理它，这会将所有的流展平为一个包含嵌套值的流。**
...
为了实现这一点，我们将把上面的流和一个初始值合并在一起，我们使用的*把它变成一个流。*
...
在我们使用 *map* 将来自后端的响应映射到处理属性之后，我们可以依次将结果映射到表情符号上以显示给我们的用户。
...
事实证明，NGXS 提供了一个调度所有动作的流，作为我们可以挂钩的可观察对象。这是我们的最终解决方案，轮询后端处理每个*添加到状态的*文档*的状态，并在后端完成处理后更新状态。*
...
*   **后端**是一个提供连接到我们后端的服务。它插入了一些必需的头等等。
