# 使用构建 messenger 应用程序。网

> 原文：<https://dev.to/neo/building-a-messenger-app-using-net--46n>
...
要跟随本教程，您将需要:
–Visual Studio，一个广泛用于构建的 IDE。NET 项目。点击查看安装细节[。
–c#基础知识。
——基础知识。净 MVC。
...
使用我们的 Visual Studio IDE，我们将按照 **New Project** 向导创建我们的聊天项目。
...
> 💡**`**View**`**函数创建一个我们返回的视图响应。当它被调用时，C#寻找调用控制器类的默认视图。这个默认视图是视图目录中的** `**index.cshtml**` **文件，该目录与控制器同名，即 HomeController 类的默认视图将是** `**Views/Home/index.cshtml**` **文件。****

 **### 设置我们的数据库

为了实现我们的登录功能，我们需要一个数据库来存储用户。有几个数据库驱动程序可供选择，但是在本教程中，我们将使用 MySQL 数据库驱动程序和一个名为实体框架的. NET ORM。

我们将从通过 NuGet 安装`MySql.Data.Entities`包开始。NET 的包管理器)。然后，我们将通过 NuGet 安装**实体**** * * * *框架* *包，为我们提供 ORM 功能。

> 💡**要使用 NuGet 安装包，在我们的项目解决方案中右键单击 packages 文件夹；选择** `**Add Package**` **选项；并搜索和选择您想要的包。**
...
> ⚠️ **您需要用实际的数据库值替换上面代码片段中的占位符值。**

`Web.config`文件是一个 **XML** 文件，上面的`connectionStrings`元素将被添加到文件的`configuration`元素的主体中。

接下来，我们将在我们的解决方案文件夹中创建一个`Models`文件夹(与`Controllers`在同一文件夹级别)。在这个文件夹中，我们将创建我们的模型类——这个类代表我们的表。对于登录功能，我们将创建`User.cs`文件。在这个类文件中，我们将添加模型的属性:
...
> 💡**创建一个模型类，右击模型文件夹，选择** `**Add**` **和** `**New File**` **选项，然后在** `**Empty Class**` **选项中填入类名。**
...
> 💡**我们是** **使用代码优先的方法实现实体** **框架 ORM。这种方法包括编写定义模型(表)的代码，而不需要任何现有的数据库或表。使用这种方法，当我们的应用程序代码被执行时，数据库和表将被创建。**

### 登录我们的用户

由于我们的数据库连接和模型(尽管随着我们的深入，可能会引入更多的模型)已经创建，我们可以继续我们的登录功能。

从`HomeController`呈现的首页将包含一个接受用户名的表单。该表单将被提交到我们之前定义的`/` `login`路线。按照我们的路由定义，这个请求将由`AuthController`和它的`Login`动作方法来处理。
...
让我们创建`ChatController`并实现包含可用联系人的聊天页面的呈现。将下面的代码粘贴到`ChatController` :
...
接下来，右击`Chat`文件夹，选择选项`Add` → `Views`，选择 Razor 模板引擎，将文件命名为`index.cshtml`。将下面的代码粘贴到文件中:
...
> 💡`**@Url.Content("~/Content/app.css")**`**`**@Url.Content("~/Content/app.js")**`**从我们的** `**Content**` **文件夹中加载一些之前捆绑的 JavaScript 和 CSS 依赖，比如 jQuery 和 Bootstrap。****
...
让我们在`Models`文件夹中创建一个名为`Conversations`的模型。它将由唯一的`id`、`sender_id`、`receiver_id`、`message`、`status`和`created_at`日期组成。模型的代码如下:
...
为了检索消息，我们将为`/contact` `/conversations/{contact}`创建一条路由。该路由将接受联系人 ID，检索当前用户和联系人之间的消息，然后在 JSON 响应中返回消息。

将由`ConversationWithContact`动作方法中的`ChatController`处理，如下图:
...
现在我们有了检索旧消息的路径，我们将使用一些 jQuery 来选择用户、获取消息并在页面上显示它们。
在我们的视图文件中，我们将创建一个`script`标签来保存我们的 JavaScript 和 jQuery 函数。在里面，我们会加上:
...
按照`RouteConfig`文件的规定，该路线将由`ChatController`的`SendMessage`动作方式处理。
...
聊天应用程序有几个需要实时功能的特性，其中一些是:
-接收实时发送的消息。
–收到即将响应的通知–“用户正在输入”功能。
–获取消息传递状态。
–联系人离线或在线时的即时通知。

在实现这些功能时，我们将使用[推杆](http://pusher.com)。要继续，让我们前往推动器[仪表板](https://dashboard.pusher.com/)并创建一个应用程序。如果你还没有账户，你可以[免费注册](https://pusher.com/signup?plan=sandbox)。用要求的信息填写创建应用程序表单。接下来，我们将使用 NuGet 在 C#代码中安装 **Pusher 服务器**包。
...
Pusher 认证请求的默认路由是`/pusher/auth`，因此我们将创建该路由并实现认证。

首先，在我们的`RouteConfig.cs`文件中，我们将添加路由定义:
...
然后，正如我们上面定义的，在`AuthController`类文件中我们将创建`AuthForChannel`动作方法并添加:
...
上面，我们的身份验证端点采用了通道的名称和客户端的套接字 ID，这是 Pusher 在连接尝试时发送的。
...
有关客户活动和私人渠道的更多信息，请查看 Pusher [文档](https://pusher.com/docs/client_api_guide/client_presence_channels)。
...
**接收实时发送的消息**
之前，我们添加了一个路由来保存用户和联系人之间的对话发送的消息。
...
在我们的`ChatController`类文件中，保存对话后，我们将添加以下内容:
...
在我们的视图中，我们将监听`new_message`事件，并使用它向我们的视图添加新消息。
...
**实现“用户正在键入”功能**
该功能使用户意识到对话是活跃的并且正在键入响应。为了实现这一点，我们将监听消息文本区域的`keyup`事件，当这个`keyup`事件发生时，我们将触发一个名为`client-is-typing`的客户端事件。
...
在 jQuery 的帮助下，我们用 C#构建了一个聊天应用程序，并使用 Pusher 实现了一些聊天应用程序中常见的实时特性。这只是在 Pusher 实时功能的帮助下构建 C#应用的一小部分。

我们很想听听你的想法，并开始用 C#和 Pusher 构建应用程序。如果你对本教程的某些部分有任何问题，我们也很乐意倾听。所以拜托，在下面留下评论吧！本教程的全部代码可以在 Github 上找到。

该帖子最先出现在 [Pusher 博客](https://blog.pusher.com/building-a-messenger-app-using-net/)上。****