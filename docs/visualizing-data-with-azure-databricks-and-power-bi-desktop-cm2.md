...
此外，我将使用 Azure Databricks 附带的一个预加载数据集，这样我就不必浪费时间解释如何将数据导入 Databricks，我在这篇[博客文章](https://dev.to/willvelida/databases-and-tables-in-azure-databricks-41hn)中对此进行了介绍。

让我们首先创建一个 Python 笔记本并加载我们的数据集。编写下面的代码:
...
首先，让我们按颜色将钻石分组，并显示它们的平均价格。我们将通过编写以下代码为此创建一个新的数据帧:
...
让我们探讨另一个例子。在数据块笔记本的另一个代码块中编写以下代码:
...
仪表板可以是非常简单的数据块。我们可以做一个快速的模拟，就像我们刚刚制作的那样，或者我们通过创建一个计划的作业来刷新它，从而增加一些复杂性。例如，如果我们创建一个为流式作业提供视觉效果的仪表板，我们可以创建一个经常更新它的作业。
...
在这里，我们看到了一系列需要连接到 Power BI 的值。我故意对你隐藏我的值，但是你需要一个以下格式的地址:
