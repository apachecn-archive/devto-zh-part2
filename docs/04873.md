...
# [](#web-app-dev-core-skill-level-1-aka-hedgehog)Web App 开发核心技能一级又名刺猬
...
### [](#has-created-an-application-that-is-independent-of-the-node-install-on-the-candidates-machine)创建了一个独立于候选人机器上的节点安装的应用程序
...
### [](#has-a-make-recipe-for-serve-which-runs-the-application-in-development-mode)有一个 make recipe for serve，它在开发模式下运行应用程序
...
### [](#has-a-make-recipe-for-test-which-runs-the-tests-for-the-application)有一个 make recipe for test，它为应用程序运行测试
...
### [](#has-a-sandbox-environment-which-allows-them-to-quickly-prototype-components)有一个沙盒环境，允许他们快速构建组件原型
...
### [](#has-the-ability-to-hotreload-based-on-changes)能够根据变化进行热重装
...
### [](#has-a-failing-test-that-appears-when-running-make-test)运行 make test 时出现测试失败
