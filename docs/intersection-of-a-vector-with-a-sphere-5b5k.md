...
> 原文：<https://dev.to/bacchu/intersection-of-a-vector-with-a-sphere-5b5k>

我的一个好朋友想在一个虚拟现实项目中得到一些帮助，看看一条光线(控制器的运动向量)是否与用户的球形视口相交。这是一个基本的 java 程序，用来检查与球体相交的向量。

用户输入输出- >交点的坐标

我已经几十年没有用 Java 编程了，所以随着程序慢慢地被构建、精炼和重构(尽我所能),这是一次有趣而丰富的经历。当然，它的计算效率不高，但希望它能成为一个在 Unity 中使用的 API！
