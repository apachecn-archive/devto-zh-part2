...
> 原文:[https://dev . to/aalises/用 puppeter-jest-n37 编写端对端测试时处理异步](https://dev.to/aalises/dealing-with-asynchrony-when-writing-end-to-end-tests-with-puppeteer--jest-n37)
...
##### [](#but-hey-here-you-have-a-great-tutorial-on-how-to-test-with-puppeteer-jest-to-get-you-started)🤘[但是，嘿，这里有一个关于如何用木偶师+ Jest 测试的很棒的教程，让你开始。](https://blog.logrocket.com/end-to-end-testing-react-apps-with-puppeteer-and-jest-ce2f414b4fd7)
...
### [](#waiting-waiting-waiting)等待，等待，等待...⏰
...
### [](#retrieving-data-from-selectors-after-an-async-operation)异步操作后从选择器中检索数据

一旦我们检测到由异步代码引起的变化，我们通常希望从我们的应用程序中提取一些数据，稍后我们可以将这些数据与来自用户交互的**预期视觉结果**进行比较。我们使用`evaluate()`来做这件事。检索数据时最常见的情况是:

#### [](#-checking-that-a-dom-element-has-appeared)-检查 DOM 元素是否出现
...
#### [](#-checking-for-matching-property-values-eg-innerhtml-option)-检查匹配的属性值(如 innerHTML，选项...)
...
### [](#intercepting-requests-and-faking-requests-with-puppeteer)用傀儡师拦截请求并伪造请求
...
### [](#getting-the-data-to-assert-from-the-raw-response-endraw-)从 [`Response`](https://pptr.dev/#?product=Puppeteer&version=v1.8.0&show=api-class-response) 获取数据进行断言
...
##### [](#note-the-raw-pageon-endraw-methods-for-request-and-response-are-tipically-placed-on-the-raw-beforeall-endraw-method-of-your-tests-after-declaring-the-page-variable-as-they-define-a-global-behavior-of-your-test)✍🏼注意:在声明页面变量之后，请求和响应的`page.on()`方法通常放在测试的`beforeAll()`方法上，因为它们定义了测试的全局行为。
...
###### [](#got-any-questions-go-out-here-start-implementing-tests-like-a-madman-and-pray-for-them-to-pass-happy-coding)有什么问题吗？！走出这里，像一个疯子一样开始实现测试并祈祷它们通过，快乐编码！🙇🏻