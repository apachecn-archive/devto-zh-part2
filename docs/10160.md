...
## [](#why-switch-away-from-vmware-and-unity-mode)为什么要放弃 VMware 和 Unity 模式？

在我介绍如何设置新的开发环境之前，我想先谈谈 VMware 设置的问题，因为我认为“为什么”和“如何”同样重要。

如果你想直接进入“如何”，那么[点击这里](#wsl-conemu-and-mobaxterm-to-the-rescue)。

#### [](#locked-into-using-xubuntu-1404)锁定为使用 Xubuntu 14.04
...
#### [](#ms-windows-doesnt-see-the-linux-driven-unity-mode-windows)MS Windows 看不到 Linux 驱动的 Unity 模式 Windows
...
## [](#wsl-conemu-and-mobaxterm-to-the-rescue)WSL、ConEmu 和 MobaXterm 前来救援
...
#### [](#setting-up-windows-subsystem-for-linux-wsl)为 Linux 设置 Windows 子系统(WSL)
...
##### [](#2-download-launch-and-install-a-distribution-of-linux)2。下载、启动并安装 Linux 发行版
...
##### [](#launching-wsl-in-the-future)未来推出 WSL
...
#### [](#setting-up-conemu-terminal-for-windows)设置 ConEmu(Windows 终端)
...
##### [](#1-heres-what-makes-conemu-a-good-choice)1。这就是 ConEmu 成为一个好选择的原因
...
#### [](#setting-up-mobaxterm-to-run-graphical-linux-apps)设置 MobaXterm 运行图形化 Linux 应用程序

MobaXterm 有很多不同的特性，但我们关心的是它可以运行 X 服务器会话。这将允许我们在 Windows 支持的窗口中呈现运行在 Linux 中的应用程序的显示。

##### [](#1-installing-and-running-mobaxterm)1。安装和运行 MobaXterm

去他们的网站下载免费版本，然后安装。

##### [](#2-make-sure-x-server-is-running)2。确保 X 服务器正在运行

你总是需要运行 MobaXterm，在应用程序的右上角有一个“X 服务器”按钮。确保它在运行。如果它说它没有运行，然后点击按钮，就是这样。

##### [](#3-set-the-display-in-wsl)3。在 WSL 中设置显示
...
##### [](#4-changing-how-the-clipboard-works)4。更改剪贴板的工作方式
...
##### [](#5a-testing-out-vscode-inside-of-wsl)5a。在 WSL 内部测试 VSCode
...
##### [](#5b-testing-out-sublime-text-inside-of-wsl)5b。在 WSL 中测试崇高的文本
...
## [](#getting-docker-up-and-running)启动并运行 Docker
