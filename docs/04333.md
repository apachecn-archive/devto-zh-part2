...
## [](#part-1-implement-metadata-fields-that-dont-need-calculation)第 1 部分:实现不需要计算的元数据字段
...
## [](#part-2-add-some-fields-that-need-calculation-and-lets-calculate-it-automatically)第二部分:添加一些需要计算的字段，让我们自动计算。
...
## [](#part-3-make-our-hands-dirtier-add-raw-postinit-endraw-only-pseudo-fields)第三部分:让我们的手更脏；仅添加`__post_init__`伪字段
...
## [](#part-4-we-dont-need-to-define-raw-authornames-endraw-)第四部分:我们不需要定义`author_names`。
...
## [](#part-5-we-need-json)第五部分:我们需要 JSON。
...
## [](#part-6-remove-unnecessary-fields-from-json)第 6 部分:移除 json 中不必要的字段。
