...
### [](#1-push-notifications-rely-on-multiple-separate-browser-apis)#1 推送通知依赖于多个独立的浏览器 API

推送通知的整个功能包括可以单独使用的推送 API 和通知 API。此外，它依赖于服务人员，因为只有他们可以在后台运行，以便能够接收推送通知，即使用户关闭了发送网站。

### [](#2-not-all-browsers-support-push-notifications-yet)#2 并非所有的浏览器都支持推送通知(还没有)
...
## [](#big-diagram-of-how-this-works)大图这是怎么回事
