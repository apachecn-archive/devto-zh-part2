...
如果您还没有安装它，[在这里](https://community.wia.io/d/4-how-to-install-node-js-on-any-raspberry-pi)学习如何安装 node.js。

# 设置树莓派

首先，你必须设置树莓派。[我们的教程将带您了解这一过程！](https://developers.wia.io/docs/raspberry-pi)一定要仔细遵循每一步。准备好之后，运行`ssh pi@('RASPBERRY-PI-IP-ADDRESS')`。这将允许您通过计算机开始在 raspberry pi 终端中工作。

# 设置 Wia Node.js SDK

首先，我们需要在 raspberry pi 上创建一个文件夹来存储我们的文件。创建一个名为`wia-wether-device`的新文件夹。

接下来，通过使用命令行`npm init`创建 packackge.json 文件来初始化这个包。对于每个提示，点击`enter`，此时它们并不重要。

一旦你完成了这些，你就可以使用命令行`npm install --save wia`将 Wia SDK 安装到 raspberry pi 上。

# 使用带有 Node.js 的 Sense HAT
...
转到 [Wia 仪表板](https://dashboard.wia.io/)并选择`Create a New Space`然后选择`Devices`。添加一个设备并给它命名。现在，在你的设备的`Configuration`标签中，你会找到以`d_sk`开头的`device_secret_key`。这在以后会很重要。
...
为了测试你的程序，现在运行`node index.js`
，你应该看到事件出现在 Wia 仪表板中你的设备的`Events`标签中。
...
接下来，创建一个网页并将其托管在 GitHub 上，这样您就可以随时从气象站查看天气了！

如果您还没有 github 帐户，[您可以在这里](https://github.com/)创建一个。

一旦设置好 github，创建一个新的存储库并命名为`your-username.github.io`。选中该框以使用自述文件进行初始化。
...
点击提交更改并访问您位于[https://github.com/username/username.github.io](https://github.com/username/username.github.io)的网站

你应该可以在你自己的网页上看到你的树莓派的天气！