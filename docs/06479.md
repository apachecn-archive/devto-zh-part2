# 学习资源

> 原文:[https://dev.to/devmazee2057282/learning-resources-3ekl](https://dev.to/devmazee2057282/learning-resources-3ekl)

## [Dev](https://dev.to)

一个关于软件开发的社区，有很多帮助和文章。

## [DevDocs](https://devdocs.io)

提供了一个简单易用的方法来搜索许多流行的文档，更多的被添加。有了*离线*支持！

## [阅读文档](https://readthedocs.org)

另一个阅读文档的网站？嗯！

## [鳄鱼](https://alligator.io)

关于前端 web 开发的文章，别忘了查看他们的资源！

## [苏格兰威士忌](https://scotch.io)

另一个文章站点，关于很多技术。

## [书呆子](https://egghead.io)

面向 web 开发人员的视频网站。

## [萨比](https://sabe.io)

关于 web 开发的文章和课程。

## [练习](https://exercism.io/)

练习，课程之类的。

## [便当](https://bento.io)

网站开发者的网站。

## [Hackr](https://hackr.io)

课程、教程和论坛。

## [大肠杆菌](https://coligo.io)

给你更多的教程。

## [【Taha shashtari】](https://tahazsh.com)

一个写 Vue.js 的人，值得一查。

## [忍者金库](https://blog.osninja.io)

一个提供技术博客文章的网站。

## [弹性网页设计](https://resilientwebdesign.com)

这太神奇了！请去读它。

## [学习编码 HTML & CSS](https://learn.shayhowe.com)

看起来真的很好，我强烈建议 HTML 和 CSS 开发人员这样做。

## [HTML，CSS & JavaScript](https://html-css-js.com)

这个我不知道还能说什么，就看看吧。

## [有用的资源](https://tympanus.net/codrops)

耶！更多资源。

## [LearnStartup](https://skills.learnstartup.net)

它有很多你可以了解的东西。构建游戏(基于网络和不基于网络)和网站。

## [跳房子](https://www.gethopscotch.com)

用游戏学习编码！

## [代码战斗](https://codecombat.com)

用游戏学习编码的另一种方式。

## [ILoveCoding](https://ilovecoding.org)

学习编码的方法。

## [编码复仇者](https://www.codeavengers.com)

我知道我想成为复仇者。一个编码复仇者！

## [维京代码学校](http://www.vikingcodeschool.com)

很酷的名字，但除此之外，来学习编码。

## [编程混帐](http://programming-motherfucker.com)

尽量不要为此生气。它实际上就是它的名字，但是你知道。为什么不去看看呢？变成混混！一个暴徒。；)

## [周五前端](https://fridayfrontend.curated.co)

每周五给你发一封关于前端的新闻信。我想。

## [开发提示](https://umaar.com/dev-tips)

在您的收件箱中获得每周开发者提示。

## [网络工具周刊](https://webtoolsweekly.com)

开发者的另一个每周收件箱。

## [后小时编程](https://www.afterhoursprogramming.com)

很多关于软件开发的教程。

## [DevShed](http://www.devshed.com)

值得一试，一个开发者社区。

## [Devdojo](https://devdojo.com)

另一个网站专注于视频网站开发。

## [站点点](https://www.sitepoint.com)

更多文章和教程。

## [教程](https://tutorialzine.com)

文章加上一个论坛社区。大部分是前端。

## [辅导点](https://www.tutorialspoint.com)

教程，教程，哦，是的，更多的教程。

## [活力逻辑](http://www.vitallogic.co.uk)

文章咄！可能不如这里的其他链接有前途。

## [【Tuts+](https://tutsplus.com)

教程！再加上更多的教程。有一些好的。

## [clever program](https://www.cleverprogrammer.com)

学习 Python。他真的很棒。

## [Scrimba](https://scrimba.com)

还有别的要查！它有视频课程和内置编辑器。一定要去看看。

## [代码学校](https://www.pluralsight.com/codeschool)

不是免费的，但是据说很不错。

## [PHP 学校](https://phpschool.io)

教你 PHP，看看吧。看起来很酷。

## [节点学校](https://nodeschool.io)

现在在 Node.js 上教你，很酷吧？

## [武学校](https://vueschool.io)

哇哦。Vue 上也有学校吗？没错。也免费！

## [【vuemastery】](https://www.vuemastery.com)

我不能忘记他们！注意，很多东西你都得付钱。但是据说真的很好。

## [W3Schools](https://w3schools.com)

网络教程，不推荐给初学者，因为这有一些过时的内容。我们现在不希望初学者使用不好的做法吧？非初学者应该足够聪明，知道什么不该相信。他们也有一些资源。

## [FreeCodeCamp](https://freecodecamp.org)

令人惊叹的教程，别忘了看看他们的中型博客。我会向编码初学者强烈推荐这个。

## [代码学院](https://www.codecademy.com)

另一个开始进入编码世界的很好的资源。我也强烈推荐给编码初学者。

## [CSS-招数](http://css-tricks.com)

伟大的提示，片段和文章。这也不是严格的 CSS 相关。

## [雄辩的 JavaScript](http://eloquentjavascript.net)

一个关于 JavaScript 的非常好的网站(书)？自己去发现吧！看起来很有希望。

## [JS:正道](http://jstherightway.org)

JavaScript 做的比以前好吗？？？

## [现代 JavaScript 教程](https://javascript.info)

这里是一些现代 JavaScript 教程！

## [PHP 妄想](https://phpdelusions.net)

一个非常好的网站，拥有最好的数据库交互教程/指南。

## [PHP 安全典范](https://paragonie.com)

虽然他们主要关注 PHP 安全，但是你仍然可以将他们的概念应用到你使用的其他语言中。仅供参考，他们真棒。目前为止关于 PHP 安全性的最好的网站或资源。

## [PHP:正道](https://www.phptherightway.com)

说真的，如果你是一个 PHP 开发人员，你真的应该读读这篇文章。它有最好的(更好的)实践。强烈推荐。

## [PHP 最佳实践](https://phpbestpractices.org)

不像正确的 PHP 那样有前途，但是应该不会太差。

## [PHP 文档](https://php.net)

你最好在这里读一些关于 PHP 的东西。

## [PHP 安全玩意儿](https://phpsecurity.readthedocs.io)

是的，更多关于 PHP 安全的内容。去看看，可能是个好主意。

## [PHP 协作标准](https://www.php-fig.org)

你绝对应该看看这个。这是标准的指导原则。

## [拉腊卡斯特](https://laracasts.com)

大量关于 web 开发的视频。有些是免费的，看看吧。

## [拉勒维尔新闻](https://laravel-news.com)

关于 Laravel 框架和包的新闻。

## [拉勒韦尔&武埃吉斯](https://www.laravel-vuejs.com)

关于 Laravel 和 Vue.js 的文章

## [黑客攻击](https://www.hacksplaining.com)

学习关于黑客的东西！挺好的，自己做的。

## [XeusHack](https://xeushack.com)

黑客教程。其他值得一查的东西。

## [精英黑客](https://elite-hackers.com)

咳咳， *sshh* 删除你的历史。驶向危险地带。或许你应该用 TOR 或者别的什么来浏览这个网站。

## [可汗学院](https://www.khanacademy.org)

有一些与软件开发相关的东西，但都是通用的。

## [Udemy](https://www.udemy.com)

有很多关于软件开发和其他东西的课程。