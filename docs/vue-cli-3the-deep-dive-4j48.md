...
> 原文：<https://dev.to/bnevilleoneill/vue-cli-3the-deep-dive-4j48>
...
vue create myproject 创建一个名为 myproject 的新项目。

vue 发球<filename>发球*。js* 或者*。零配置开发模式下的 vue* 文件。</filename>

vue build <filename>从*构建生产就绪包。js* 或者*。零配置的 vue* 文件。</filename>

vue invoke <pluginname>调用已安装的插件生成器来创建插件工作所需的文件。</pluginname>

vue inspect 显示应用程序的 webpack 配置，因为它已经完全抽象出来了。
...
要使用 CLI 从先前保存的预置创建新项目，请将目录更改为预置的位置，并运行下面的代码
...
其中@vue/pwa 是您要添加的插件的名称，在本例中是 Progressive Web App 插件。
...
新的 CLI 让服务*变得超级简单。vue* 或*。使用 vue serve 和 vue build 命令在开发或生产模式下创建 js* 文件。
...
serve 命令还提供了一个选项，可以在运行完命令后启动浏览器，而不是只显示它当前为应用程序提供服务的 url。
...
要构建为库，vue build -t lib app.vue 而要构建为 web 组件，vue build -t wc app.vue。
...
开发模式的. env.development 和生产模式的. env.production。

**注意:**对于将变量嵌入客户端捆绑包代码的 CLI，变量需要以 VUE 应用名称为前缀。
...
为此，您必须使用 vue.config.js
中的 configureWebpack 选项
...
这就是 vue inspect 命令派上用场的地方。

该命令将所有内部 Webpack 配置输出到您的终端。

要将其输出到一个文件，只需指定一个文件名，如下所示
...
它可以与任何应用程序完美配合，不管是什么框架，并且有插件可以记录来自 Redux、Vuex 和@ngrx/store 的额外上下文。
