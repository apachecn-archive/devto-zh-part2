...
> 原文:[https://dev . to/kena kamu/azure-vs-GCP-part-12-virtual-machine-scaling-GCP-2op 0](https://dev.to/kenakamu/azure-vs-gcp-part-12-virtual-machine---scaling-gcp--2op0)
...
# [](#managed-instance-groups)托管实例组

一个组包含相同的虚拟机，并提供自动扩展。当负载超过阈值时，它使用**实例模板**作为种子映像来添加额外的虚拟机。有两种类型的托管实例组。

### [](#zonal-managed-instance-group)分区管理实例组
...
### [](#regional-managed-instance-group)区域托管实例组
...
### [](#global-external-load-balancing)全局外部负载均衡
...
### [](#regional-externalinternal-load-balancing)区域外部/内部负载均衡
...
### [](#create-instance-template)创建实例模板
