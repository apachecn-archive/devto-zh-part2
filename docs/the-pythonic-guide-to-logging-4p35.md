...
> 原文：<https://dev.to/timber/the-pythonic-guide-to-logging-4p35>

如果处理得当，日志是您的可观察性套件的一个有价值的组件。日志讲述了数据在应用程序中是如何变化的。他们让你回答这样的问题:为什么 John Doe 昨天结账时收到一个错误？他提供了哪些输入？

日志记录的实践范围从非常简单的静态字符串语句到丰富的结构化数据事件。在本文中，我们将探索 Python 中的日志记录如何让您更好地了解应用程序中正在发生的事情。此外，我将探索一些最佳实践，帮助您从日志中获得最大价值。

### print('为什么不直接用 print 语句？')

许多 Python 教程向读者展示了如何使用`print`作为调试工具。这里有一个使用`print`作为脚本打印异常的例子:
...
我相信学习的最好方法是去做，所以我鼓励你跟随 Python REPL。开始使用日志模块很简单，您需要做的就是:
...
注意`info`和`debug`没有打印信息。默认情况下，记录器将只打印`warning`、`error`或`critical`消息。您可以定制这种行为，甚至在运行时修改它，以动态激活更详细的日志记录。

```
>>> # should show up; info is a higher level than debug >>> logger.setLevel(logging.DEBUG)
>>> logger.info(1)
INFO:root:1
>>>
>>> # shouldn't show up; info is a lower level than warning ... logger.setLevel(logging.WARNING)
>>> logger.info(2) 
```
...
其中一些数据，比如`time`和`levelname`可以自动捕获，但是您可以(也应该)将`extra`上下文推送到您的日志中。

### 添加上下文

一般日志消息提供的信息和没有日志消息提供的信息一样少。想象一下，如果你必须检查你的日志，你看到了`removed from cart`。这使得很难回答这样的问题:*该项目是什么时候被删除的？谁移除了它？他们拿走了什么？*
...
日志记录引入了开销，需要与您编写的软件的性能需求进行平衡。虽然开销通常可以忽略不计，但糟糕的实践和错误会导致[不幸的情况](https://www.youtube.com/watch?v=uyIlAO390v4)。
...
如果你一直在跟进(你应该这样)，你会注意到你的日志不会出现在你的文件中，因为它是第`info`级。确保使用`setLevel`来改变这种情况。

现在使用`grep`可以很容易地搜索日志文件。
...
现在，您可以搜索包含关键字`critical`或`warning`的邮件。
...
# automatically deletes logs more than 5 days old because `backupCount=5` handler = TimedRotatingFileHandler(pathToLog, when="d", interval=1,  backupCount=5) 
```
...
在观察您的应用程序时，理解日志的作用非常重要。我建议读者看一看可观察性原则的 3 个支柱，因为日志应该是你在观察应用程序时使用的少数工具之一。日志是可观察性栈的一个组成部分，但是度量和跟踪同样如此。
...
我们有一个非常棒的服务 [here @ Timber](https://timber.io/) (真的很棒)，它可以自动捕捉日志的上下文，让调试变得更容易，而不是自己去做。免费试用我们；你甚至不需要信用卡！
