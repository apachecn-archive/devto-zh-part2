...
> 原文：<https://dev.to/neo/build-a-cryptocurrency-alert-app-using-kotlin-and-go-part-1the-frontend-2h9d>
...
*   机器上安装的 Android Studio(v3 . x 或更高版本)。[下载这里](https://developer.android.com/studio/index.html)。
...
首先，启动 Android Studio 并创建一个新的应用程序。输入应用程序的名称，例如 **CryptoAlert** ，然后输入包名。确保选择了**启用 Kotlin 支持**复选框。选择最小 SDK，点击**下一步，**选择一个**空活动**模板，坚持主活动命名方案，然后点击**完成**。

#### 创建一个推梁应用程序

由于推杆梁依赖于 Firebase，我们需要一个 FCM 密钥和一个`google-services.json`文件。到你的 [Firebase 控制台](https://console.firebase.google.com/u/0/)并点击**添加项目**卡来初始化应用创建向导。

添加项目名称，例如`crypto-` `alert`，阅读并接受条件条款。此后，您将被引导至项目概述屏幕。选择**添加 Firebase 到你的 Android 应用**选项。

下一个屏幕将要求您的应用程序的包名。你可以在你的 app-module `build.gradle`文件中找到你的应用的包名。注意`applicationId`值。输入包名，点击**下一步**。你会被提示下载一个`google-services.json`文件。下载文件并跳过剩余的过程。将下载的文件添加到您的项目- `name-of-project/app`的 app 文件夹中。
...
创建实例后，您将看到一个快速入门指南。选择**安卓快速启动**。添加 FCM 密钥后，您可以退出快速入门指南。

#### 为我们的应用添加功能

为了让我们的应用程序工作，我们需要引入几个依赖项。为此，
将以下内容添加到项目`build-gradle`文件中:
...
接下来，将以下内容添加到应用程序模块`build.gradle`文件:
...
接下来，我们将在应用程序中管理通知。创建一个名为
`NotificationsMessagingService`的新服务，并粘贴这个:
...
我们还利用存储在`strings.xml`文件中的一些文件来描述通知通道描述和通道名称。将这些添加到`strings.xml`文件:
...
现在，让我们准备我们的布局。首先，我们将设计活动的布局。创建应用程序时，`activity_main.xml`文件应该已经存在于布局文件夹中。打开它，换成这个:
...
布局包含两个`TextView`来显示比特币和以太坊的价格。我们还使这些`TextView`可点击，这样我们可以设置限制，以便在超过限制时获得通知。

接下来，我们将设计警报对话框的布局。创建一个名为`alert_layout`的新布局文件，并粘贴此文件:
...
这将是对话框显示的布局。它包含两个文本字段和一个**保存**按钮，我们为`TextView` s 使用了一个自定义设计的背景。创建一个名为`text_background`的新的可绘制文件并粘贴这个:
...
我们将转到`MainActivity`来完成我们应用程序的功能。打开您的`MainActivity`，将内容替换为以下内容:
...
上面，我们创建了一个用于网络调用的`retrofit`对象。设置好`retrofit`对象后，我们在`onCreate`方法中添加布局，并调用其他三个函数:

1.  `fetchCurrentPrice` -该函数将从我们的服务器获取比特币和以太坊的当前价格。在类中创建一个新函数，并像这样设置它:

    //File:/app/src/main/Java/{ package-name }/main activity . kt
    私趣 fetchCurrentPrice(){
    retrowing . get values()。enqueue(object:Callback {
    override fun on response(Call:Call？，回应:回应？){
    val jsonObject = JSONObject(响应！！。body())
    bitcoinvalue . text = " 1 BTC = $ "+JSON object . getjson object(" BTC ")。getString(" USD ")
    etherumvalue . text = " 1 ETH = $ "+JSON object . getjson object(" ETH ")。getString("USD")
    }
...
1.  `setupPushNotifications` -此功能是我们开始收听我们选择接收通知的兴趣所在。利息名称是这样的格式**{ device _ uuid } _ { currency } _ changed**。我们注册两个兴趣，每种货币一个。打开`MainActivity`类并添加以下方法:

    //File:/app/src/main/Java/{ package-name }/main activity。Kt
    私趣 setupPushNotifications(){
    push notifications . start(application context，" PUSHER _ BEAMS _ INSTANCE _ ID ")
    val fmt = " % s _ % s _ changed "
    push notifications . subscribe(Java . lang . string . format(fmt，deviceUuid()，" BTC)))
    push notifications . subscribe(Java . lang . string . format(fmt，deviceUuid()，" ETH)))
    }

> 用推杆梁仪表板上的实例 ID 替换`PUSHER_BEAMS_INSTANCE_ID`。

1.  在这个函数中，我们将在我们的布局中设置文本视图的点击监听器。在同一个`MainActivity`类中，添加以下方法:

    //File:/app/src/main/Java/{ package-name }/main activity。Kt
    私趣 setupClickListeners(){
...
当任何文本视图被点击时，我们调用`createDialog`方法，然后打开一个布局，让用户输入限制。
...
在发送时，我们还会发送设备的唯一标识符`uuid`。让我们创建`deviceUuid()`方法，它将为每个设备生成并保存这个 UUID。在`MainActivity`类中，添加以下代码:
...
申请到此为止。此时，应用程序应该能够成功构建，但是不能按预期运行。在下一部分的[中，我们将构建应用程序的后端，以便它能够按预期工作。](https://pusher.com/tutorials/cryptocurrency-kotlin-go-part-2)

### 结论

在本文中，我们学习了如何使用推送光束来通知用户对加密货币的更改。您可以在这里找到本文[中构建的应用程序的存储库。](https://github.com/neoighodaro/kotlin-cryptocurrency-watcher-with-push-notification)
