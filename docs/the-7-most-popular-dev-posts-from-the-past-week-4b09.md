# 过去一周最受欢迎的 7 篇开发人员帖子

> 原文：<https://dev.to/devteam/the-7-most-popular-dev-posts-from-the-past-week-4b09>

# 1。Docker 默认

Docker 会像 Git 成为版本控制的默认选择一样成为容器的默认选择吗？

[![thejoezack](img/fe6ce7f767c65cae3e9a14c9268b0500.png)](/thejoezack) [## Docker 是新的饭桶吗？

### 乔扎克 4 月 9 日 182 分钟阅读

#docker #git #devops](/thejoezack/is-docker-the-new-git-h5i)

# 2。🎩✨Get 搞定了✨🎩

谁能抗拒一些好的生产力技巧呢？Christian 分享了他们从特殊按键到电子邮件工作流程的技巧。

[![fabrik42](img/f065a139c0ee7751269d916f23ee5ce8.png)](/fabrik42) [## 我的生产力助推器——技巧和工具的随机集合——你的是什么？

### 克里斯汀

#discuss #productivity #devtips #tips](/fabrik42/my-productivity-boosters--a-random-collection-of-tricks-and-tools--what-are-yours-28fm)

# 3。就这么干吧

亨利·朱分享他成为巴别塔全职维护者的经历。在这篇文章中，他谈到了建立自信和练习自我遗忘。

[![hzoo](img/649eb1ee0cc18d20dc28eaaee930ac11.png)](/hzoo) [## 我还没准备好成为巴别塔的维护者

### 亨利 Apr 10 ' 18 分钟阅读

#opensource #javascript #babel #career](/hzoo/i-was-not-ready-to-become-the-maintainer-of-babel-2j6)

# 4。Postgres vs MySQL

顾名思义，这是对 Postgres 和 MySQL 的一次非常彻底的比较——从模式布局到类型差异再到查询数据。

[![dmfay](img/f28422448075068c9600498f2ea2f435.png)](/dmfay) [## 终极 Postgres vs MySQL 博客文章

### Dian Fay Apr 11 ' 1812 分钟阅读

#database #postgres #mysql](/dmfay/the-ultimate-postgres-vs-mysql-blog-post-1l5f)

# 5。像开发者一样学习

学习每一项技术是不可能的，但是像开发人员一样学习并在需要时快速理解如何使用新技术是可能的。

## 文章不再可用

# 6。令人愉快的框架

Lynne 最近开始与 Vue.js 合作，并分享了一系列你应该尝试一下的理由。

[![lynnewritescode](img/cc944ec8ba246e024078c951343f6206.png)](/lynnewritescode) [## 使用 Vue.js 的 6 个理由

### 琳恩·芬尼根

#vue #javascript #coding #technology](/lynnewritescode/6-reasons-to-use-vuejs-1p6c)

# 7。WSL 设置

约翰利用 Windows 10 的 creators 更新，买了一台电脑，而不是花钱买一台 Mac。这是他为 Linux 设置的 windows 子系统。

## 文章不再可用

我们的每周总结到此结束！请关注 dev.to 本周的每日内容和讨论...如果您遗漏了任何内容，我们将在下周一进行回顾！