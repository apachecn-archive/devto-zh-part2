...
> 原文：<https://dev.to/oktadev/build-user-registration-with-node-react-and-okta-3944>
...
然后在客户机应用程序中为 API 设置代理，这样您仍然可以调用`/api/{resource}`并将其从端口 3000 代理到端口 3001。在`client/package.json`文件中，在`name` :
下添加`proxy`设置
...
或者，如果你正在使用[纱](https://yarnpkg.com)包管理器:
...
将名为`app.config.js`的文件添加到`client/src`文件夹中。文件的内容有:
...
然后设置`index.js`文件使用 React 路由器和 Okta 的 React SDK。当`index.js`文件完成时，它将看起来像这样:
...
同样，您使用的是`withAuth`高阶组件。对于每个需要使用 Okta 的认证或授权过程的组件来说，这将是一个反复出现的主题。在本例中，它主要用于获取`isAuthenticated`函数。在构造函数和`componentDidUpdate`生命周期方法中执行`checkAuthentication()`方法，以确保在创建组件时对其进行检查，并对组件的每个后续更改进行再次检查。

当`isAuthenticated`返回 true 时，那么它被设置在组件的状态中。然后在 render 方法中检查它，以决定是显示`LoginForm`组件，还是重定向到用户的个人资料页面，这是您接下来要创建的组件。

现在在`auth`文件夹中创建`ProfilePage.js`组件。组件的内容有:
...
现在，您已经有了可以处理所有路由的组件，可以创建路由了。更新`App.js`文件，使最终版本看起来像:
...
在 Node 应用程序根目录下的`app.js`文件中，更新文件以将所有呼叫路由到`/api/<something>`。您会在`app.use`语句块下面看到一个部分。更改设置的路线，使其看起来像这样:
...
和往常一样，如果你对这篇文章有任何问题、评论或担忧，你可以在下面发表评论，给我发电子邮件到[lee.brandt@okta.com](mailto:lee.brandt@okta.com)，或者把你的问题发到[开发者论坛](https://devforum.okta.com)。更多文章和教程，请关注我们的 Twitter [@OktaDev](https://twitter.com/oktadev) 。