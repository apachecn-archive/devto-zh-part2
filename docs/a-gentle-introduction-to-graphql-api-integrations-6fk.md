...
> 原文：<https://dev.to/hugo__df/a-gentle-introduction-to-graphql-api-integrations-6fk>

GraphQL 是 REST(或其他 HTTP API 设计)的一个很好的替代品。这是关于使用 GraphQL API 的核心概念的快速介绍。

要查看使用 GraphQL API 的一些示例:

*   在 Python 中，参见[使用 gql 的 Python GraphQL 客户端请求示例](https://dev.to/hugo__df/python-graphql-client-requests-example-using-gql-523g)
*   在 JavaScript(浏览器和节点)中，[参见上周的代码与雨果简讯](https://buttondown.email/hugo/archive/9011fe42-c8f7-4201-8b38-d777c1f6910a)

## graph QL 是什么，它解决什么问题？
...
第一种情况导致必须进行大量调用来获取所有数据，第二种情况导致巨大的负载和缓慢的加载时间。
...
我们希望在查询中返回什么，参见[“字段”的 GraphQL 文档](https://graphql.org/learn/queries/#fields/)。返回字段`name`、`fleeRate`、`maxCP`、`maxHP`的 GraphQL 查询如下(参见这里的输出[):](https://graphql-pokemon.now.sh/?query=%7B%0A%20%20pokemon(name%3A%20%22Pikachu%22)%20%7B%0A%20%20%20%20name%0A%20%20%20%20fleeRate%0A%20%20%20%20maxCP%0A%20%20%20%20maxHP%0A%20%20%7D%0A%7D%0A) 
...
我们将如何向下过滤查询数据，参见[graph QL 文档中的“arguments”](https://graphql.org/learn/queries/#arguments)。为了得到前 10 个口袋妖怪的名字，我们使用了`pokemons (first: 10) { FIELDS }`见输出[这里](https://graphql-pokemon.now.sh/?query=%7B%0A%20%20pokemons%20(first%3A%2010)%20%7B%0A%20%20%20%20name%0A%20%20%20%20fleeRate%0A%20%20%20%20maxCP%0A%20%20%20%20maxHP%0A%20%20%7D%0A%7D%0A) :
...
别名使我们能够重命名字段，参见 GraphQL 文档中的“别名”。我们实际上要用它来映射查询中的字段，例如从骆驼到蛇:
...
要在类型上扩展的字段的定义。这是一种保持查询干燥的方式，通常会分离出重复、重用或深度嵌套的字段定义，参见[graph QL 文档中的片段](https://graphql.org/learn/queries/#fragments)。这将意味着不做([见](https://graphql-pokemon.now.sh/?query=%0A%7B%0A%20%20pokemon(name%3A%20%22Pikachu%22)%20%7B%0A%20%20%20%20id%0A%20%20%20%20number%0A%20%20%20%20weight%20%7B%0A%20%20%20%20%20%20minimum%0A%20%20%20%20%20%20maximum%0A%20%20%20%20%7D%0A%20%20%20%20height%20%7B%0A%20%20%20%20%20%20minimum%0A%20%20%20%20%20%20maximum%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D) [这里的查询动作](https://graphql-pokemon.now.sh/?query=%0A%7B%0A%20%20pokemon(name%3A%20%22Pikachu%22)%20%7B%0A%20%20%20%20weight%20%7B%0A%20%20%20%20%20%20minimum%0A%20%20%20%20%20%20maximum%0A%20%20%20%20%7D%0A%20%20%20%20height%20%7B%0A%20%20%20%20%20%20minimum%0A%20%20%20%20%20%20maximum%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D) ):
...
以下形式的查询([见图一](https://graphql-pokemon.now.sh/?query=%7B%0A%20%20pokemons(first%3A%205)%20%7B%0A%20%20%20%20name%0A%20%20%7D%0A%7D) ):
...
以下形式的查询([见图一](https://graphql-pokemon.now.sh/?query=%7B%0A%20%20pokemon(name%3A%20%22Pikachu%22)%20%7B%0A%20%20%20%20name%0A%20%20%20%20classification%0A%20%20%7D%0A%7D) ):
...
以下形式的查询([见图一](https://graphql-pokemon.now.sh/?query=%7B%0A%20%20pokemon(id%3A%20%22UG9rZW1vbjowMjU%3D%22)%20%7B%0A%20%20%20%20name%0A%20%20%20%20classification%0A%20%20%7D%0A%7D) ):
