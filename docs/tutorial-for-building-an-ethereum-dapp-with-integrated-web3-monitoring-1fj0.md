...
> 原文：<https://dev.to/xngwng/tutorial-for-building-an-ethereum-dapp-with-integrated-web3-monitoring-1fj0>
...
*   *你设计好你的智能合约了吗？_ 智能合约为您的 DApp 定义了规则和转换，并且是在以太坊虚拟机中运行的部分。避免在你的智能合约中加入不必要的逻辑，因为运行其计算的气体*可能非常昂贵。我们将从 truffle 的简单智能契约开始，称为 _SimpleStorage。_ 它存储一个无符号整数`storedData`，并提供一个 setter 和 getter。
...
该表单将允许用户设置他们希望存储在 SimpleStorage 契约中的值。按钮的动作处理程序在这里:
