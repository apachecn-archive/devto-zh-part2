# 2017 年 Web 组件统计

> 原文:[https://dev . to/this isbinh/web-components-statistics-in 2017-2 ECF](https://dev.to/thisisbinh/web-components-statistics-in2017-2ecf)

本文是“测试 696 个 Web 组件”系列的最后一篇文章。本文中的统计数据涵盖了许多主题:webcomponents.org 上发布的组件数量、基于其库的组件分布、工作和非工作组件，以及一些更有趣的统计数据。

注:这些统计数据大约是一年前得出的。其中一些在今天可能变得无关紧要。

### [](#300-elements-in-3-months-thats-an-impressive-number)3 个月 300 个元素。这是一个令人印象深刻的数字。

[![](../Images/606c4d082f0e2befb7e2f8862d90550c.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--7_Qcz4Bn--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/rzvu8nmg4kovi609kaw1.png) 
让我们把这些数字分解一下:

*   聚合物始终占出版元素的 90%以上。很明显，Polymer 在 web 组件库领域占据主导地位。
*   每月大约有 100 个定制元素发布，数量惊人。
*   这里最显著的变化是聚合物 1 和 2 组分的分布。聚合物 2 有 278 个元素的激增，而聚合物 1 大致保持不变。

### [](#out-of-696-tested-elements-here-is-how-many-are-functioning)在 696 个被测试的元素中，这里显示了有多少正在运行

[![](../Images/9be971916b919869b1d77b0444469c31.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--a7jJFjUT--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/r1hdp7wc2lwpylhc1cwz.png) 
**平视！**每个工作组件都有自己的现场演示和截图供你在 Vaadin 目录下试用。

### [](#browser-compatibility)浏览器兼容性

在 489 个工作元素中，以下是每个浏览器中的工作元素数量:
[![](../Images/2496a999cbe1001750d0f16e70527db5.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--HA67HY49--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/l4mqk6uoyhy3xffmr0cc.png)

### [](#conclusion)结论

毫无疑问，事情朝着有利于 web 组件的方向发展。越来越多的公司和技术将赌注压在它身上。我希望你喜欢这个系列。如果你没有读过前两篇，下面是:

## 文章不再可用

## 文章不再可用

如果您有任何关于测试 web 组件的问题，请随时 PM 或给我发电子邮件。感谢阅读！

*   *更多关于 Web 组件的新闻和文章，请在 Twitter 上关注我 [@binhbbbb](https://twitter.com/binhbbbb)*
*   如果你想尝试 Web 组件，请查看系列 [Web 组件星期三](https://vaadin.com/blog?tag=wcw)，在那里我向初学者介绍并解释了易于使用的 Web 组件。