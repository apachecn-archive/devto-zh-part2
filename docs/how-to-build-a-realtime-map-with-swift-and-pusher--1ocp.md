# 如何用 Swift 和 Pusher 创建实时地图

> 原文：<https://dev.to/neo/how-to-build-a-realtime-map-with-swift-and-pusher--1ocp>
...
> ⚠️ **用你的应用程序的名字替换** `**application_name**` **。**

运行下面的命令开始安装我们在`Podfile` :
中指定的包
...
创建一个新目录来保存我们的 Node.js 应用程序。打开终端并`cd`到 Node.js 应用程序的目录。在这个目录中，创建一个新的`package.json`文件。打开该文件并粘贴下面的 JSON:
...
在目录中创建一个新的`index.js`文件，并将下面的代码粘贴到文件中:
...
在上面的控制器类中，我们导入了所有需要的库。然后我们实例化这个类的一些属性。在`viewDidLoad`方法中，我们设置了`mapView`上的坐标，并给它添加了`locationMarker`。
...
> 💡**我们攻击的端点的 URL([http://localhost:3000/simulate](http://localhost:3000/simulate))是一个本地 web 服务器。这意味着在构建真实案例时，您将需要更改端点 URL。**
...
> 💡**你需要把** `**GOOGLE_IOS_API_KEY**` **替换成你创建 Google iOS API 密钥时得到的密钥。**

在同一个文件的顶部，查找`import UIKit`，并在它下面添加以下内容:
...
要运行 Node.js 应用程序，请使用您的终端将`cd`添加到 Node.js 应用程序目录，并运行以下命令来启动 Node 应用程序:
...
在本文中，我们看到了如何使用 Pusher 和 Swift 在 iOS 上构建实时地图。希望你学到了一些如何创建实时 iOS 应用程序的东西。如果你有任何问题或建议，请在下面留下评论。

本教程的源代码可以在 [GitHub](https://github.com/neoighodaro/Sample-Realtime-Map-on-iOS) 上获得。

这篇文章最初出现在 [Pusher 博客](https://blog.pusher.com/realtime-map-swift-pusher/)上。