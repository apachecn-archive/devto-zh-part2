...
> 原文:[https://dev . to/app signal/解构-elixirs-genservers-4h8a](https://dev.to/appsignal/deconstructing-elixirs-genservers-4h8a)
...
## [](#genserver-internals)发电服务器内部

为了理解我们的键值服务器是如何工作的，我们将深入到`GenServer`模块中。它依靠*消息传递*和*递归*进行通信和保持状态。

### [](#message-passing-between-processes)流程间的消息传递
...
### [](#state-through-recursion)状态通过递归实现
...
## [](#implementing-our-own-genserver)实现我们自己的 GenServer
...
## [](#using-genservers-in-elixir)在仙丹中使用基因服务器
