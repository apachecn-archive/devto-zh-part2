# Vim:如何使用多个剪贴板在类固醇上进行复制和粘贴

> 原文：<https://dev.to/strikingloo/vim-how-to-use-multiple-clipboards-to-do-copy-and-paste-on-steroids-262a>
...
本教程假设你已经熟悉了 Vim 的基础知识，所以如果你不知道从哪里开始，我建议你看看这篇文章。
...
*   按*Ctrl+w*(Mac 中的 *cmd + w* )。
*   按下 *v* (垂直)或 *h* (水平)。

这将创建一个垂直拼接，或水平拼接。这意味着文件将在当前窗口之外的另一个“窗口”中打开(在同一终端内)，光标在同一行。

垂直或水平是屏幕的两种可能的“分割”,因此 *Ctrl + w v* 将在并排的两个窗口中打开文件，而 *Ctrl + w h* 将在一个窗口下面打开另一个窗口。

要在使用完一个窗口后将其关闭，只需按下:q 即可退出。

要在一个新的 Vim 窗口中打开一个不同的文件，我们可以键入 *:vsplit <文件名>* 来有效地从一个终端编辑许多不同的文件。
...
在这种情况下，按下 *Ctrl + W* ，然后按下 *h、j、k* 或 *l* ，如果你经常这样做，可能会有点不舒服。我发现许多人做的事情(我自己也这样做)是在他们的。vimrc 文件:
...
我发现按下 Ctrl + h，j，k，l 比整个序列更流畅，而且大多数时候你只是在几个窗口之间切换。当然，如果你喜欢不同的快捷方式，那就试试吧！这就是这个编辑器的妙处。
...
我撒了一点谎。不是预留缓冲，而是很多。要决定将东西放入或取出哪个缓冲器，请按“然后键入*【A-Z】、【A-Z】*或*【0–9】*。

例如，如果我们正在编辑一个文本，发现自己经常键入单词“特定”、“特别”和“文本编辑器”，我们可以这样做:

*   按下 *v* 进入视觉模式，通过移动光标选择单词(在本例中，我们使用“特定”)
*   按下*”p*选择 *p* 寄存器，然后按下 *y* 将其拉入。
*   每当我们不得不再次键入单词时，只需按下 *"p* 说出从哪个寄存器放入，然后按下 *p.*

如果我们希望看到寄存器的内容，我们可以运行 *:reg* 来列出它们，或者 _ :reg_ 后跟一个用空格分隔的寄存器名称列表(例如 _ :reg a b_)来单独查看每一个。
...
要清除 Vim 寄存器的内容，如果 *r* 是您的寄存器名称，请按 *qrq* 。
...
*关注我的* [*中的*](http://www.medium.com/@strikingloo) *和* [*推特*](http://twitter.com/strikingloo) *继续阅读更多面向开发者和数据科学家的教程、技巧和窍门。如果您喜欢这篇文章，请与一位程序员朋友分享！*

*原载于*[*www . datastuff . tech*](http://www.datastuff.tech/programming/vim-squeezing-text-editors-juice-with-more-features/)
