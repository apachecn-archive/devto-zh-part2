...
> 原文：<https://dev.to/oktadev/create-and-verify-jwts-with-node-o8o>

这些年来，互联网上的身份认证有了很大的发展。有很多方法可以做到这一点，但在 90 年代行之有效的方法在今天并不完全适用。在本教程中，我将简要介绍一些更古老、更简单的身份验证形式，然后向您展示一种更现代、更安全的方法。在本文结束时，您将能够在 Node 中自己创建和验证 jwt。我还将向您展示如何利用 [Okta](https://developer.okta.com/) 在幕后为您完成这一切。
...
[Okta](https://developer.okta.com/) 是一种云服务，允许开发者创建、编辑和安全存储用户账户和用户账户数据，并将它们与一个或多个应用程序连接。我们的 API 使您能够:
...
接下来，登录到您的开发人员控制台，导航到**应用**，然后单击* *添加应用**。选择**网页**，然后点击**下一步**。给你的应用起一个名字，比如“JWTs 的乐趣”。将**基地 URI** 改为`http://localhost:3000/`，将**登录重定向 URI** 至`http://localhost:3000/implicit/callback`，然后点击**完成**。
...
*   如果你的 JWT 被偷了会怎么样？
*   [使用 OAuth 2.0 客户端证书保护节点 API](/blog/2018/04/24/simple-node-authentication)
*   [教程:用 Java 创建和验证 JWTs】](/blog/2018/10/31/jwts-with-java)
*   [简单节点认证](/blog/2018/04/24/simple-node-authentication)
*   [为什么 jwt 不适合作为会话令牌](/blog/2017/08/17/why-jwts-suck-as-session-tokens)
