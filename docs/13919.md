...
## [](#redux-and-state-management)Redux 和状态管理
...
### [](#managing-state-with-actions-and-reducers)用动作和还原器管理状态
...
### [](#required-fields-easy-html5-validations)必填字段:简易 HTML5 验证
...
### [](#why-we-shouldnt-validate-in-the-component)为什么我们不应该在组件中进行验证

我们可以通过在`AstronautForm`中向我们的`submitForm`函数添加代码来直接在组件中处理这个问题:

#### [](#bad-example-dont-do-this)坏榜样，不要这样:
...
### [](#tracking-errors-in-reduxs-state)在 Redux 状态下跟踪错误
...
### [](#custom-validation-middleware)自定义验证中间件
...
#### [](#adding-to-the-middleware-stack)添加到中间件堆栈
...
#### [](#performing-the-validations)执行验证
...
#### [](#validate-on-state-change)状态改变时验证
