...
> 原文:[https://dev.to/nuster/nuster-a-高性能-http-代理-缓存-服务器-和-restful-no SQL-缓存-服务器-基于-haproxy - 4aa2](https://dev.to/nuster/nuster---a-high-performance-http-proxy-cache-server-and-restful-nosql-cache-server-based-on-haproxy--4aa2)
...
### [](#as-httptcp-loader-balancer)作为 HTTP/TCP 加载器的平衡器
...
### [](#as-http-cache-server)作为 HTTP 缓存服务器
...
### [](#as-restful-nosql-cache-server)作为 RESTful NoSQL 缓存服务器
...
## [](#as-tcp-loader-balancer)作为 TCP 加载器的平衡器
...
## [](#as-httphttps-loader-balancer)作为 HTTP/HTTPS 加载器的平衡器
...
## [](#global-nuster-cachenosql)全局:n 主缓存|nosql
...
## [](#proxy-nuster-cachenosql)代理:主机缓存|nosql
...
## [](#proxy-nuster-rule)代理:nuster 规则
...
### [](#wait-onofftime-cache-only)等待开|关|时间【仅缓存】
...
### [](#code-code1code2)代码 CODE1，代码 CODE2...
...
### [](#lastmodified-onoff)最后修改开|关

启用上次修改的条件请求处理。如果没有，添加`Last-Modified`标题。

默认关闭。

### [](#ifunless-condition)如果|除非条件
...
## [](#enable-and-disable-rule)启用和禁用规则
...
### [](#update-state-and-ttl)更新状态和 TTL
...
### [](#advanced-purging-purge-by-name)高级清除:按名称清除
...
### [](#advanced-purging-purge-by-host)高级清除:由主机清除
...
### [](#advanced-purging-purge-by-path)高级清除:按路径清除
...
### [](#advanced-purging-purge-by-regex)高级清除:按正则表达式清除
...
## [](#cache-nustercachehit-boolean)【缓存】nuster.cache.hit:布尔值
...
## [](#cannot-start-not-in-masterworker-mode)无法启动:不在主从模式下
...
## [](#how-to-cache-post-request)如何缓存 POST 请求？
...
## [](#how-to-enable-http2)如何启用 HTTP2
