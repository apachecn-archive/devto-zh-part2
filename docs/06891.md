...
> 原文:[https://dev.to/nickparsons/containerizing-a-nodejs-api-使用-docker-with-kubernetes-and-mini kube-16o 9](https://dev.to/nickparsons/containerizing-a-nodejs-api--using-docker-with-kubernetes-and-minikube-16o9)
...
## [](#step-5-kubernetes-cli)步骤 5:不间断 CLI
...
# [](#cloning-the-boilerplate-express-api)克隆样板 Express API💾
...
## [](#next-move-into-the-directory-build-the-api-and-launch-it-to-test)接下来，进入目录，构建 API 并启动它进行测试:
...
# [](#containerizing-the-api-with-docker)用 Docker 容器化 API📦
...
## [](#step-1-understanding-the-dockerfile-a-template-already-exists-within-the-directory)第一步:了解 Dockerfile(目录中已经存在一个模板):
...
## [](#step-2-building-a-docker-image)第二步:构建 Docker 镜像:

```
docker build -t boilerplate-api/api . 
```

## [](#step-3-list-all-docker-images-to-verify-yours-exists)第三步:列出所有 Docker 图片(验证您的图片是否存在):
...
## [](#step-4-run-the-docker-image)第四步:运行 Docker 镜像:
...
## [](#step-5-view-running-api)第五步:查看正在运行的 API:
...
# [](#stepping-into-the-minikube-amp-kubernetes-land)踏入迷你库贝&库贝内斯的土地👣

Minikube 是一个让本地运行 Kubernetes 变得容易的工具。Minikube 在笔记本电脑上的虚拟机中运行单节点 Kubernetes 集群。

## [](#setting-up-minikube)设置 Minikube
...
# [](#start-the-minikube-dashboard)启动 Minikube 仪表盘
...
# [](#create-a-kubernetes-deployment)创建一个 Kubernetes 部署💻
