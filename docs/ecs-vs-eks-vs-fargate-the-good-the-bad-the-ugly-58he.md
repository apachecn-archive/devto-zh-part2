# ECS Vs. EKS Vs .法盖特:好的，坏的，丑陋的

> 原文：<https://dev.to/totalcloudio/ecs-vs-eks-vs-fargate-the-good-the-bad-the-ugly-58he>

“你是码头工人还是库伯内特人？”

“如果你喜欢使用 Docker，AWS ECS 就是为你准备的！”

“如果你喜欢库伯内特，AWS EKS 就是你的选择！”

如果你不想做管理 Docker 或 Kubernetes 的繁重工作，AWS Fargate 就是为你准备的！

> 虽然我们已经从云工程师的几个账户中听到了这样的陈述，但令人惊讶的是，这些服务在顶层看起来都很相似，但都有自己的特点和优势。([点击这里查看这幅漫画😆](https://blog.totalcloud.io/choosing-container-environment-on-aws-ecs-eks-or-fargate/))。

为了让你快速浏览一下，这里有一个表格，里面有一些关键的策划事实。

# ECS vs EKS vs 法盖特一言以蔽之

[![ECS Vs. EKS Vs. Fargate: The Good, the Bad, the Ugly](img/78f91c17c6279974b7b8e5f938990047.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--gDbcq_bQ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/http://blog.totalcloud.io/wp-content/uploads/2018/06/ECS_EKS_FARGATE_GOOD_BAD_UGLY-V2.jpg)T4】

[![ECS Vs. EKS Vs. Fargate](img/e3758de0e6ea58b95cb012f724916f28.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--WhoFV9Ll--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/ozfd6psoi2bhgezsnekl.jpg)

如果你认为我们错过了任何好的方面，请发推文给我们，地址是 [@totalcloudio](https://twitter.com/totalcloudio) 。

感谢在 [数据库](http://www.databasable.com) 的马克·韦弗，他帮助我们策划了他在与这些服务机构合作时所做的一些有趣的观察。

###### 参考文献:

###### [https://aws.amazon.com/eks/pricing/](https://aws.amazon.com/eks/pricing/)

###### [https://aws.amazon.com/ecs/pricing/](https://aws.amazon.com/ecs/pricing/)

###### [https://aws.amazon.com/fargate/faqs/](https://aws.amazon.com/fargate/faqs/)

###### [https://docs . AWS . Amazon . com/eks/latest/user guide/service _ limits . htmlT3】](https://docs.aws.amazon.com/eks/latest/userguide/service_limits.html)

###### [https://medium . com/becloudy/Amazon-container-cheat-sheet-d5d 05469181 c](https://medium.com/becloudy/amazon-container-cheat-sheet-d5d05469181c)

###### [https://www . Reddit . com/r/AWS/comments/87r 6 uo/understanding _ amazons _ container _ services _ ECS _ eks/](https://www.reddit.com/r/aws/comments/87r6uo/understanding_amazons_container_services_ecs_eks/)

注:最初发布于 2018 年 6 月 29 日 [blog.totalcloud.io](https://blog.totalcloud.io/ecs-vs-eks-vs-fargate-good-bad-ugly/) 。