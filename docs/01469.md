...
> 原文:[https://dev . to/pongsatt/deploy-static-front-end-application-to-kubernetes-cluster-using-Jenkins-CD-and-minio-with-automatic-https-cert-44gk](https://dev.to/pongsatt/deploy-static-front-end-application-to-kubernetes-cluster-using-jenkins-cd-and-minio-with-automatic-https-cert-44gk)
...
# [](#setup-minio)设置 minio

**注:**

> minio 需要访问密钥和密码。请用您自己的值替换“myaccesskey”和“mysecret”

### [](#1-install-minio-using-helm)1。使用 helm 安装 minio
...
### [](#2-install-minio-web-client)2。安装 minio web 客户端
...
### [](#3-configure-minio-pointing-to-the-server)3。配置指向服务器的 minio
...
### [](#4-create-minio-folder-to-host-static-files)4。创建 minio 文件夹来存放静态文件
...
# [](#build-a-static-website)建立一个静态网站

我们将使用 react 应用程序作为我们的静态网站示例。让我们建造它。

### [](#1-initialize-application)1。初始化应用程序
...
### [](#2-add-kubernetes-ingress-configuration)2。添加 kubernetes 入口配置
...
### [](#3-add-jenkinsfile-pipeline)3。添加 Jenkinsfile 管道
...
### [](#4-publish-to-git-repo)4。发布到 git repo

您可以将您的项目发布到任何 git 存储库。

# [](#prepare-jenkins-environment)准备詹金斯环境
...
# [](#build-jenkins-pipeline)建立詹金斯管道

在本例中，我们将创建指向存储库中 Jenkinsfile 的 Jenkins 管道。

### [](#1-create-jenkins-pipeline-project)1。创建詹金斯管道项目
