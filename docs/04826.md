...
## [](#example-node-app-prettyprintexe)示例节点 app 'prettyprint.exe '
...
### [](#module-that-exports-our-function)模块，导出我们的函数
...
### [](#a-dummy-json-file-to-test-if-everything-works)一个虚拟的 JSON 文件来测试是否一切正常
...
### [](#test-if-you-copypasted-properly)测试复制/粘贴是否正确
...
### [](#add-one-property-to-packagejson)向 package.json 添加一个属性
...
#### [](#simple-way-to-build-for-current-platform-and-version)针对当前平台和版本的简单构建方式

从你的应用文件夹中，运行 *pkg -t host。*。 *-t* 表示目标平台，值 *host* 表示您的系统。*。*表示当前目录。
显然，您可以运行 *pkg - help* 来获得参数的完整列表。

#### [](#in-packagejson-main-and-bin-need-not-be-different)在 package.json 中，“main”和“bin”不必不同

虽然您通常希望将它们分开，但是 *main* 和 *bin* 可以具有相同的值，并且不一定需要是两个单独的文件。

#### [](#dependencies-need-to-be-in-packagejson)依赖项需要在 package.json 中

如果在创建应用程序后进行 NPM 安装，它会自动将依赖项添加到 package.json 中。

#### [](#native-modules-and-assets)原生模块和资产
...
##### [](#make-your-own-standalone-compiler-executable)制作自己的独立编译器可执行文件
