...
> 原文:[https://dev . to/nioperas 06/build-email-verification-from-scratch-with-mason ite-framework-and-JSON-we b-tokens-mf7](https://dev.to/nioperas06/build-email-verification-from-scratch-with-masonite-framework-and-json-web-tokens-mf7)
...
## [](#create-new-masonite-project-and-scaffold-authentication)创建新的 Masonite 项目和脚手架认证
...
## [](#create-new-database-and-put-your-database-credentials-into-env-file)创建新的数据库并将你的数据库凭证放入。环境文件:
...
## [](#add-active-attribute-to-user-model)为用户模型添加“活动”属性
...
## [](#why-use-jwt-for-email-verification)为什么使用 JWT 进行邮件验证？

JSON Web 令牌是一种在各方之间安全传输信息的好方法。

对于电子邮件验证，我们需要向注册用户发送一个随机散列。

## [](#generate-new-jwt-token-on-registration)注册时生成新的 jwt 令牌
...
## [](#create-a-route-for-email-verification)创建邮件验证路径
...
## [](#define-fillable-attribute-activate-on-user-model)定义用户模型上的可填充属性‘激活’
...
## [](#last-thing-login-users)最后一件事！登录用户:
...
## [](#your-user-is-now-verified)您的用户现已通过验证🔥
