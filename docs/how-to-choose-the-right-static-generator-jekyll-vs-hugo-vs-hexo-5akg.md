...
> 原文：<https://dev.to/techiediaries/how-to-choose-the-right-static-generator-jekyll-vs-hugo-vs-hexo-5akg>
...
*   简单易用
*   多亏了 [Jekyll importers](https://import.jekyllrb.com/docs/home/) ，你可以轻松地从流行平台(如 WordPress)迁移你的内容
*   强大的 Github 页面支持
*   带有默认和体面的最小化的开箱即用的主题
...
使用 Jekyll，只需几个命令，您就可以在几秒钟内启动并运行。一旦你有了一台安装了 Ruby 和 gem 的开发机器，你就可以运行下面的命令:
...
你也可以使用通过[官方插件](https://github.com/jekyll/jekyll-textile-converter)支持的*纺织*。

#### 宝石题材

Jekyll 允许你使用主题作为宝石，可以从 RubyGems 安装，这有很多好处

> 基于 Gem 的主题使得主题开发者可以很容易地向任何拥有 gem 主题的人提供更新。当有更新时，主题开发者将更新推送到 RubyGems。[来源](https://jekyllrb.com/docs/themes/#understanding-gem-based-themes)

### 降价和多种格式支持

Jekyll 使用流行的 [Markdown](https://daringfireball.net/projects/markdown/) 格式， [YAML](https://jekyllrb.com/docs/frontmatter/) 用于前台，CSS 和 HTML 用于格式化静态内容。
...
您可以通过向 build 命令添加- incremental 开关来启用实验性的增量构建特性。

### 资产处理

Jekyll 有一个支持*萨斯*和 *SCSS* 开箱即用的资产流水线。它还允许您通过一些配置选项以多种方式定制 Sass 预处理，例如指定原始 Sass/SCSS 文件夹(默认为`sass_dir`)和设置 Sass 支持的输出样式。

你也可以通过添加官方插件 [jekyll-coffeescript](https://github.com/jekyll/jekyll-coffeescript) 来添加对 *Coffeescript* 的支持。
...
一旦你创建了一个网站，你就可以开始在`_posts`文件夹中添加文章。您也可以在`_posts`中创建子文件夹。

你可以通过在根文件夹或子文件夹中放置一个 HTML/Markdown 文件来添加一个页面，这是任何网站的基本构建。

集合是 Jekyll 最近增加的。你可以从[杰基尔文档](https://jekyllrb.com/docs/collections/)中找到更多关于收藏的信息

> 集合允许您定义一种新的文档类型，其行为类似于页面或帖子的正常行为，但也有自己独特的属性和名称空间

你可以通过在`_drafts`文件夹中放置没有日期的帖子来使用[草稿](https://jekyllrb.com/docs/drafts/)，然后你可以使用带有`jekyll serve`或`jekyll build`的- drafts 开关来预览你的草稿。

您也可以使用 Jekyll 处理数据文件。从[文档](https://jekyllrb.com/docs/datafiles/)中查找更多信息

> Jekyll 支持从位于 _data 目录中的 YAML、JSON 和 CSV 文件加载数据。请注意，CSV 文件必须包含标题行。

### 插件和扩展性

Jekyll 有一个强大的[插件系统](https://jekyllrb.com/docs/plugins/)，它带有钩子，允许你为你的站点创建定制的内容。这允许您在站点生成过程的不同点运行代码。

你可以找到用于各种任务的[开源插件](https://planetjekyll.github.io/plugins/)，官方的或社区创建的，这样你就不必重新发明轮子，如果你熟悉 te Ruby 语言，你也可以创建自己的插件。
...
Hugo 没有 Jekyll 那么受欢迎，但它现在是网络上最受欢迎的三个静态生成器之一。它被视为杰基尔的快速替代品。

### 工作流程和易用性

你可以为 Hugo 下载一个预编译的二进制文件，不用安装 go 和其他依赖项就可以开始使用。

> 有很多关于“雨果被写成 go”的说法，但你不需要安装 Go 就能欣赏雨果。随便抓一个预编译的二进制！[来源](https://gohugo.io/getting-started/installing/)

如果你喜欢软件包管理器，你也可以用一个命令为主要的操作系统安装 Hugo:
...
接下来添加一些内容并运行本地服务器，然后导航到`http://localhost:1313/`。
...
你可以在这里找到更多信息[。](https://gohugo.io/getting-started/quick-start/)
...
Hugo 没有资产管道，因此您必须使用外部工具来处理需要预处理的资产，例如 *ES6* 、Sass 或更少等等。

如果你不想使用外部资产管道，你必须坚持使用普通的 JavaScript 和 CSS，因为在构建过程中，Hugo 只将文件从静态文件夹复制到构建文件夹。

### 降价和多种格式支持

Hugo 支持 Markdown 和开箱即用的 Emacs 组织模式。使用外部助手可以支持诸如 Asciidoc 之类的其他格式。你可以在这里找到更多[的信息。](https://gohugo.io/content-management/formats/)
...
在我写这篇文章的时候，Hexo 已经在 Github 上有了 *20.336* 颗星星。
...
Hexo 也允许你在`source`文件夹中有全局资产文件夹。

您可以使用不同的标签来轻松引用不同资产文件夹中的资产，例如 asset_path、asset_img 和 asset_link。

### 模板引擎和主题

Hexo 使用*嵌入式 JavaScript 模板* (EJS)一种简单的[模板语言](http://ejs.co/)，让你用普通的 JavaScript 生成 HTML 标记。它支持布局，局部和局部变量。

Hexo 引入了*片段缓存*，这是一个受 [Ruby on Rails](http://guides.rubyonrails.org/caching_with_rails.html#fragment-caching) 启发的特性。它允许将内容保存和缓存为片段，从而加快文件生成过程。

你可以很容易地构建一个 Hexo 主题，你只需要创建一个新的文件夹，并指定一个[结构](https://hexo.io/docs/themes.html)。要开始使用您的主题，请在您站点的`_config.yml`中修改主题设置。或者你也可以使用社区创建的开源和免费的[主题](https://hexo.io/themes/)。
...
Hexo 有一个专注于博客的内容管理系统，支持国际化。您可以[创建帖子](https://hexo.io/docs/writing.html)和草稿作为降价文件，并为元数据提供前置内容。

Hexo 支持[数据文件](https://hexo.io/docs/data-files.html)，它从你的帖子中的`source/_data`加载 YAML 或 JSON 文件。

### 插件和扩展性

Hexo 是高度可扩展的，有太多的可用插件，并且支持所有 Octopress 插件和许多 Jekyll 插件。

总而言之，Hexo 易于安装和使用，速度非常快，高度可扩展。它没有内置的预处理 Sass，ES6 等。但它可以很容易地集成为 Gulp 或 Grunt 的一部分，以使用外部资产预处理工具。Hexo 是维护博客的一个很好的选择。

## 其他专业化的替代品:盖茨比和 Gitbook

### 盖茨比

Gatsby 是一个基于 React 和 GraphQL(用于从不同来源提取数据，如 headless CMS 系统、SaaS 服务、API、数据库或文件系统等)的现代高速静态站点生成器。)
...
*   用强大的工具创作书籍(用 Markdown 编写的内容)
*   可以生成多种格式的电子书:PDF，EPUB 等。
*   书籍由 Git 管理
*   你可以通过[自助出版平台](https://www.gitbook.com/)发行你的书
*   它支持插件，并且有数百个免费的插件用于不同的目的
