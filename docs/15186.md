...
## [](#but-what-is-nodejs)但是，Node.js 是什么？
...
## [](#top-7-nodejs-mistakes-to-avoid)避免 7 大 Node.js 错误

### [](#1-initiating-multiple-callbacks)1。发起多次回调

有时，开发人员会保存一个文件，然后重新加载 Node.js web 应用程序,看看它是否会很快崩溃。这会导致多次回调执行，因为它们在第一次执行后会忘记返回。JavaScript 依赖回调来帮助它连接各种异步元素，使它们正常工作。

### [](#2-not-testing-properly)2。测试不正确
...
### [](#5-blocking-of-the-event-loop)5。阻塞事件循环
