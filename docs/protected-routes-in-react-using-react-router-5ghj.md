# 使用 React 路由器在 React 中保护路由

> 原文：<https://dev.to/theoutlander/protected-routes-in-react-using-react-router-5ghj>

在本视频中，我们将学习使用 React 路由器创建受保护的路由。只有当用户登录时，才能访问此路线。

[链接到 CodeSandbox](https://codesandbox.io/s/ol6z72kjy9)
[链接到 Youtube 上的视频](https://www.youtube.com/watch?v=Gp2KAqNvOFQ)

* * *

#### 如果这个视频有帮助，❤️ it 和[订阅我的 YouTube 频道](https://www.youtube.com/channel/UClCm-WAOgN6XLwCpefSVezg?sub_confirmation=1)。

[![video camera](img/162cbaf8d6272b73c3862dce71e9fe2e.png) 25:38 ](/theoutlander/react-component-lifecycle-explained-3koh) [![theoutlander](img/fa3a5404d413070afd491a6351af2493.png)](/theoutlander) [## React 组件生命周期解释

### 尼克卡尼克 8 月 14 日 181 分钟阅读

#react #javascript #webdev #video](/theoutlander/react-component-lifecycle-explained-3koh)

* * *