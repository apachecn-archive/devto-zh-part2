...
# [](#everyday-promiselike-functions)日常承诺类功能
...
# [](#converting-to-raw-async-endraw-raw-await-endraw-)转换为`async` / `await`
...
# [](#unified-return-interface-with-raw-async-endraw-raw-await-endraw-)与`async` / `await`统一返回界面
