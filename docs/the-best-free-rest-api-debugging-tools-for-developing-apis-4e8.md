...
> 原文：<https://dev.to/xngwng/the-best-free-rest-api-debugging-tools-for-developing-apis-4e8>
...
### ApiRequest。超正析象管(Image Orthicon)
...
和 httpbin 一样， [mocky.io](https://mocky.io) 是[开源](https://github.com/julien-lafont/Mocky)，也有托管版本。Mocky 比以前的工具更加灵活，因为它允许任意的响应体和状态代码。如果您需要将某些特定的 JSON 返回给您的客户机，这一点很重要。它还支持各种各样的内容类型，如文本/xml、多部分和 HTML。此外，您可以设置预定义的延迟，如`?mocky-delay=100ms`。这是伟大的测试超时条件或当添加加载指标到一个网站。
...
有相当少的付费性能测试服务，这是有意义的，因为建立性能测试需要做更多的工作。因此，免费服务往往涉及更多的工作，尤其是对于托管服务，它可能是资源密集型的。
...
许多 API，尤其是支持单页面应用的私有 API，利用[跨源资源共享](https://dev.to/blog/technical/restful-apis/Authorization-on-RESTful-APIs/)来安全地对不同的域进行 AJAX 调用。
...
如果我们错过了您最喜欢的有助于更好地使用 API 的工具，请在下面提出您的建议或意见。
