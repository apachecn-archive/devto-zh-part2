...
### [](#drawlinex1-as-integer-y1-as-integer-x2-as-integer-y2-as-integer)画线(x1 为整数，y1 为整数，x2 为整数，y2 为整数)

从位置 x1，y1 到位置 x2，y2 绘制一条直线。

### [](#drawovalx-as-integer-y-as-integer-w-as-integer-h-as-integer)DrawOval(x 为整数，y 为整数，w 为整数，h 为整数)

在 x，y 位置绘制一个椭圆，使用 w 和高度 h。要绘制一个圆，使 w 和 h 相同。

### [](#drawpicturex-as-integer-y-as-integer-picturepath-as-string)DrawPicture(x 为整数，y 为整数，picturePath 为字符串)

在指定路径的 x，y 位置画图。

### [](#drawrectx-as-integer-y-as-integer-w-as-integer-h-as-integer)DrawRect(x 为整数，y 为整数，w 为整数，h 为整数)

使用宽度 w 和高度 h 在 x，y 位置绘制一个矩形。

### [](#drawstrings-as-string-x-as-integer-y-as-integer-w-as-integer-0-condense-as-boolean-false)拉带(s 为字符串，x 为整数，y 为整数，w 为整数= 0，condense 为 Boolean = False)

在图形屏幕上的 x，y 位置绘制一个字符串 s。可以选择指定一个宽度，以及如果字符串不适合时是否压缩。

### [](#endofline-as-string)EndOfLine 为字符串

用于字符串的行尾字符。

### [](#endtimer-as-double)EndTimer 为 Double

停止由 StartTimer 启动的计时器，返回经过时间的毫秒数。

### [](#fillovalx-as-integer-y-as-integer-w-as-integer-h-as-integer)FillOval(x 为整数，y 为整数，w 为整数，h 为整数)

在 x，y 位置绘制一个实心椭圆，使用 w 和高度 h。要绘制一个圆，使 w 和 h 相同。使用 ForeColor 命令更改填充颜色。

### [](#fillrectx-as-integer-y-as-integer-w-as-integer-h-as-integer)FillRect(x 为整数，y 为整数，w 为整数，h 为整数)

使用宽度 w 和高度 h 在 x，y 位置绘制填充矩形矩形，使用 ForeColor 命令更改填充颜色。

### [](#forecolorassigns-c-as-color)前景色(指定 c 为颜色)

设置用于绘制和填充命令的颜色。

### [](#gpiodigitalreadpin-as-integer-as-integer)GPIODigitalRead(pin 为整数)为整数

从 GPIO 引脚读取值。

### [](#gpiodigitalwritepin-as-integer-mode-as-integer)GPIODigitalWrite(引脚为整数，模式为整数)

向 GPIO 引脚写入一个值。

### [](#gpiopinmodepin-as-integer-mode-as-integer)GPIOPinMode(引脚为整数，模式为整数)

将 GPIO 引脚设置为输入或输出。

初始化 GPIO。 [WiringPi 库](http://wiringpi.com)必须安装在 Pi 上。

### [](#gpiosofttonecreatepin-as-integer-as-integer)GPIOSoftToneCreate(pin 为整数)为整数

为大头针设置音调。

### [](#gpiosofttonewritepin-as-integer-freq-as-integer)GPIOSoftToneWrite(pin 为整数，freq 为整数)
...
### [](#usercancelled-as-boolean)用户取消为布尔型

当用户单击工具栏上的“停止”按钮时，返回 True。用它来结束长循环运行的程序。

### [](#waitms-as-double-10)等待(ms As Double = 10)

暂停程序执行。默认值为 10 毫秒，但是您可以指定自己的值。

## [](#learn-more-about-xojo-and-raspberry-pi)了解更多关于 Xojo 和树莓派的信息
