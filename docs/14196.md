...
> 原文:[https://dev.to/charrondev/getting-to-know-quilljs-第一部分-羊皮纸-印迹-和-生命周期- 3e76](https://dev.to/charrondev/getting-to-know-quilljs---part-1-parchment-blots-and-lifecycle--3e76)
...
#### [](#-raw-blotcreate-endraw-)`Blot.create()`
...
#### [](#-raw-constructordomnode-endraw-)`constructor(domNode)`
...
#### [](#ensuring-blots-have-unique-identifiers)确保印迹具有唯一的标识符
...
### [](#insertion-and-attachment)插入和附着

既然已经创建了一个污点，我们需要将它附加到 Quill 的文档树和 DOM 树中。有多种方法可以在文档中插入污点。

#### [](#-raw-insertintoparentblot-refblot-endraw-)`insertInto(parentBlot, refBlot)`
...
#### [](#-raw-insertatindex-name-value-endraw-)`insertAt(index, name, value)`
...
#### [](#-raw-updatemutations-mutationrecord-sharedcontext-object-endraw-)`update(mutations: MutationRecord[], sharedContext: Object)`
...
#### [](#-raw-optimizecontext-endraw-)`optimize(context)`
...
### [](#deletion-and-detachment)删除和脱离

#### [](#-raw-remove-endraw-)`remove()`

`remove()`方法通常是完全删除污点及其 DOM 节点的最简单方法。它从 DOM 树中移除污点的`.domNode`，然后调用`detach()`。

#### [](#-raw-removechildblot-endraw-)`removeChild(blot)`

这个方法只在`ContainerBlot`及其子类上可用。从调用印迹的`.children`中移除传递的印迹。

#### [](#-raw-deleteat-endraw-)`deleteAt()`

删除指定索引处的污点或内容。在内部调用`remove()`。

#### [](#-raw-detach-endraw-)`detach()`
