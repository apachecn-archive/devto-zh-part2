...
> 原文:[https://dev.to/bugsnag/anatomy-of-a-javascript-error-51kc](https://dev.to/bugsnag/anatomy-of-a-javascript-error-51kc)
...
### [](#-raw-syntaxerror-endraw-)`SyntaxError`
...
### [](#-raw-referenceerror-endraw-)`ReferenceError`
...
## [](#manually-throwing-errors)手动投掷错误
...
## [](#intercepting-errors-using-trycatch)使用 try/catch 拦截错误
...
## [](#gotta-catch-em-all-with-global-error-handling)必须通过全局错误处理来捕捉所有错误

即使是最努力编写的代码，错误有时仍然会出现。没事的。错误时有发生。当他们这样做的时候，尽快知道是很重要的。这就是 Bugsnag 这样的错误报告工具的用武之地。

### [](#how-global-error-handling-works)全局错误处理的工作原理
...
## [](#creating-custom-error-types)创建自定义错误类型
